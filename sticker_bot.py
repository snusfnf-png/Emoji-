...
async def _do_copy_pack(update_or_msg, context: ContextTypes.DEFAULT_TYPE,
    ...
    for i, src_sticker in enumerate(stickers):
        ...
        # ПРИЧИНА ПРОПУСКА 1: Telegram иногда возвращает RetryAfter на get_file.
        # Нужно читать retry_after из исключения и ждать точно столько.
        raw = None
        for dl_attempt in range(5):
            try:
                f   = await context.bot.get_file(src_sticker.file_id)
                buf = io.BytesIO()
                await f.download_to_memory(buf)
                buf.seek(0)
                raw = buf.read()
                if raw:
                    break
                # Пустой ответ — подождём и попробуем снова
                await asyncio.sleep(2)
            except TelegramError as dl_err:
                err_s = str(dl_err)
                # Читаем точное время ожидания из RetryAfter если есть
                ...
                if retry_after:
                    ...
                    # Любая другая TG ошибка — тоже ретраим (FILE_REFERENCE_EXPIRED и т.п.)
                    wait = 3 * (dl_attempt + 1)
        ...
        # ── Конвертация ───────────────────────────────────────────────────────
        # ПРИЧИНА ПРОПУСКА 2: Конвертация падала молча → стикер пропускался.
        # Теперь при ошибке конвертации пробуем fallback (raw без обработки).
        try:

...
def _admin_main_kb():
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("📅 Новые по дням",      callback_data="adm_daily")],
...
async def adm_stats(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    text = (
        ...
        f"🖼 Стикер-паков создано: <b>{stats.get('sticker_packs_created', 0)}</b>\n"
        f"✨ Эмодзи-паков создано: <b>{stats.get('emoji_packs_created', 0)}</b>\n"
        f"🚗 Номерных знаков: <b>{stats.get('plates_created', 0)}</b>\n"
        f"➕ Стикеров добавлено в паки: <b>{stats.get('stickers_added', 0)}</b>\n"
        f"➕ Эмодзи добавлено в паки: <b>{stats.get('emoji_added', 0)}</b>\n"
    )
    await q.edit_message_text(
        text, parse_mode="HTML",
        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("◀️ Назад", callback_data="adm_back")]]),
...
async def adm_daily(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if not rows:
        ...
        return ADMIN_MENU
    lines = ["📅 <b>Новые пользователи по дням</b>\n"]
    for day, cnt in rows:
        lines.append(f"• {day} — <b>{cnt}</b> чел.")
    await q.edit_message_text(

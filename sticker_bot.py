...
def _find_or_download_kz_flag() -> str | None:
    """Скачивает PNG флага Казахстана из noto-emoji и возвращает путь к файлу."""
    import urllib.request
    ...
    if os.path.exists(flag_path):
        return flag_path
    try:
        logger.info("Скачиваю флаг КЗ: %s", flag_path)
        urllib.request.urlretrieve(
            "https://github.com/googlefonts/noto-emoji/raw/main/png/128/emoji_u1f1f0_1f1ff.png",
            flag_path,
        )
        return flag_path
    except Exception as e:
        logger.warning("Не удалось скачать флаг КЗ: %s", e)
        return None
    ...
    if country == "RU":
        ...
        if KZ_FLAG_PATH and os.path.exists(KZ_FLAG_PATH):
            try:
                ...
                ratio    = target_flag_h / flag_img.height
                resized  = flag_img.resize(
                    (max(1, int(flag_img.width * ratio)), target_flag_h), Image.LANCZOS)

    ...
    try:
        if mode == "single":
            ...
            if data["is_animated"]:
                ...
                # Оставляем WebP — не конвертируем в PNG, Telegram принимает WebP напрямую
                fname = "sticker.webp"
            ...
            # Для отправки одиночного стикера в чат — создаём временный пак
            # (работает и для TGS и для WebP, т.к. send_sticker напрямую ненадёжен)
            ...
            try:
                await create_set(tmp_name, f"Recolor [{hex_color}]", inp, fmt, stype)
            except Exception as e:
                es = str(e).lower()

    ...
    try:
        if mode == "single":
            ...
            if pack_created:
                ...
                # Сразу сохраняем покрашенный пак в "Мои паки"
                _paint_save_to_packs(context, uid, new_pack_name, new_pack_title,
                                     "emoji" if is_emoji_pack else "sticker", ok)
                await msg.reply_text(
                    ...
                    reply_markup=InlineKeyboardMarkup([
                        [InlineKeyboardButton("💾 Сохранить", callback_data="list_packs")],
...
def _paint_save_to_packs(context, user_id: int, pack_name: str, pack_title: str,
                          pack_type: str, count: int):
    """Сохраняет покрашенный пак в список паков пользователя (если ещё нет)."""
...
async def _do_copy_pack(update_or_msg, context: ContextTypes.DEFAULT_TYPE,
    ...
    if creating_new:
        # Сохраняем сразу — как в покраске стикеров
        _paint_save_to_packs(context, update.effective_user.id,
                             target_pack_name, target_pack_title, target_type, added)
        kb.append([InlineKeyboardButton("💾 Сохранить", callback_data="list_packs")])
...
# copy_save_pack удалена — сохранение теперь происходит сразу, кнопка ведёт напрямую в list_packs

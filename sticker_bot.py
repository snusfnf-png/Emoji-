...
def convert_to_webm(raw_bytes: bytes) -> bytes | None:
    """Конвертирует MP4/GIF в WEBM (VP9) для видео-стикера Telegram.
    Telegram требует: WEBM VP9, строго 512x512, макс 256кб, макс 3 сек.
    """
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        try:
            ...
            logger.info("convert_to_webm: output size=%d bytes", len(data))
            return data
...
async def get_sticker_data(bot, msg, pack_type: str = "sticker"):
    ...
    if msg.document:
        ...
        # GIF → конвертируем в WEBM для анимированного стикера
        if "gif" in mime or name.endswith(".gif"):
            raw = await dl(d.file_id)
            webm = convert_to_webm(raw)
            if webm:

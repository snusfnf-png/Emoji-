...
async def _ask_emoji_for_next(update_or_msg, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    # Если уже подтверждено "везде 🔥" — применяем огонёк автоматически
    if context.user_data.get("fire_emoji_confirmed_new"):
        # Автоматически применяем 🔥 без вопроса
        emoji_list = ["🔥"]
        ...
        data      = file_info["data"]
        fmt       = file_info["fmt"]
        is_custom_emoji = file_info.get("is_custom_emoji", False)
        ...
        return await _ask_emoji_for_next(update_or_msg, context)
...
async def fire_all_yes_new(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Подтверждено 'везде 🔥' для нового пака — применяем к текущему и всем следующим."""
    query = update.callback_query
    await query.answer()
    ...
    # Применяем к текущему стикеру в очереди
    return await _ask_emoji_for_next(update, context)

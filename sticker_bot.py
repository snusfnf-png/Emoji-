...
def generate_plate_image(country: str, chars: str, region: str, region_name: str) -> bytes:
    ...
    if country == "RU":
        ...
        fnt_r = ImageFont.truetype(FONT_BOLD, 36)
        draw.text((rcx, py + 34), region, fill="#111111", font=fnt_r, anchor="mm")
        # 2. RUS + flag on ONE line below region
        fw, fh = 26, 17
        # total width of flag+gap+text for centering
        fnt_rus = ImageFont.truetype(FONT_BOLD, 13)
        rus_w = int(fnt_rus.getlength("RUS"))
        gap = 4
        total_w = fw + gap + rus_w
        row_y = py + 68
        fx = rcx - total_w // 2
        fy = row_y - fh // 2
        _ru_flag(draw, fx, fy, fw=fw, fh=fh)
        draw.text((fx + fw + gap, row_y), "RUS", fill="#111111", font=fnt_rus, anchor="lm")

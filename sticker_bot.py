...
def recolor_webp(raw: bytes, hex_color: str) -> bytes:
    """
    HSV Hue-rotation recolor.
    Меняем только оттенок (Hue) пикселя на оттенок целевого цвета.
    Насыщенность (S) и яркость (V) оригинала сохраняются — тени и блики не трогаются.
    """
    h = hex_color.lstrip("#")
    tr, tg, tb = int(h[0:2], 16) / 255, int(h[2:4], 16) / 255, int(h[4:6], 16) / 255
    target_hue, _, _ = colorsys.rgb_to_hsv(tr, tg, tb)
    ...
    v = cmax
    s = np.where(cmax > 0.001, delta / cmax, 0.0)

    # нейтральные (чёрные, белые, серые) почти не трогаем
    color_weight = np.clip((s - 0.15) / 0.25, 0.0, 1.0)

    nh = np.full_like(v, target_hue)
    i_sector = (nh * 6).astype(np.int32) % 6
    f = nh * 6 - np.floor(nh * 6)
    p = v * (1 - s)
    q = v * (1 - f * s)
    t = v * (1 - (1 - f) * s)

    new_r = np.select([i_sector == 0, i_sector == 1, i_sector == 2,
                       i_sector == 3, i_sector == 4, i_sector == 5],
                      [v, q, p, p, t, v])
    new_g = np.select([i_sector == 0, i_sector == 1, i_sector == 2,
                       i_sector == 3, i_sector == 4, i_sector == 5],
                      [t, v, v, q, p, p])
    new_b = np.select([i_sector == 0, i_sector == 1, i_sector == 2,
                       i_sector == 3, i_sector == 4, i_sector == 5],
                      [p, p, t, v, v, q])

    final_r = r * (1 - color_weight) + new_r * color_weight
    final_g = g * (1 - color_weight) + new_g * color_weight
    final_b = b * (1 - color_weight) + new_b * color_weight

    result = arr.copy()
    result[:, :, 0] = np.clip(final_r * 255, 0, 255).astype(np.uint8)
    result[:, :, 1] = np.clip(final_g * 255, 0, 255).astype(np.uint8)
    result[:, :, 2] = np.clip(final_b * 255, 0, 255).astype(np.uint8)
    ...
    try:
        if mode == "single":
            ...
            if data["is_animated"]:
                ...
                # Определяем тип: кастомный эмодзи или обычный стикер
                ...
                try:
                    ...
                    # Если пак уже существует — добавляем в него
        ...
        try:
            await status.edit_text(
                f"❌ Ошибка: <code>{_friendly_tg_error(exc)}</code>",
                parse_mode="HTML",
            )
        except Exception:
            pass

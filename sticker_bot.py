...
async def receive_sticker_file(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    try:
        ...
        logger.info("receive_sticker_file: pack_type=%s", pack_type)
...
async def _push_sticker_to_tg(bot, user_id: int, pack_name: str, pack_type: str,
    ...
    def make_file():
        """Создаёт свежий InputFile из bytes каждый раз."""
        if isinstance(data, bytes):
            buf = io.BytesIO(data)
            buf.seek(0)
            return InputFile(buf, filename=fname)
        return data  # уже file_id строка

    # PTB 20.7: InputSticker НЕ принимает format — его нет в __init__
    # Проверяем версию API динамически
    import inspect as _inspect
    _has_fmt = "format" in _inspect.signature(InputSticker.__init__).parameters
    logger.info("InputSticker has 'format' param: %s", _has_fmt)

    def make_sticker():
        if _has_fmt:
            return InputSticker(sticker=make_file(), emoji_list=clean_emoji[:20], format=fmt)
        else:
            return InputSticker(sticker=make_file(), emoji_list=clean_emoji[:20])

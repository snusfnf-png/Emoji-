...
def generate_plate_image(country: str, chars: str, region: str, region_name: str) -> bytes:
    ...
    if country == "RU":
        ...
        # ── Флаг Беларуси (точные пропорции) ──────────────────────────────
        zone_w = 82    # ширина зоны флага внутри пластины
        fl_w   = 66    # ширина флага
        fl_h   = 44    # высота флага (3:2)
        ...
        # Флаг + зазор 4px + "BY" — всё вместе центрируем вертикально
        total_h = fl_h + 4 + by_h
        fl_x    = px + (zone_w - fl_w) // 2
        fl_y    = py + (ph - total_h) // 2

        red_h = round(fl_h * 2 / 3)   # красная полоса — 2/3
        grn_h = fl_h - red_h           # зелёная  — 1/3
        ...
        # Белая орнаментальная полоска слева (1/9 ширины флага)
        orn_w = max(6, fl_w // 9)
        draw.rectangle([fl_x, fl_y, fl_x+orn_w, fl_y+fl_h], fill="white")

        # Традиционный белорусский орнамент — ромбы
        d   = max(4, orn_w - 1)   # высота одного ромба
        ocx = fl_x + orn_w // 2   # центр X полоски
        for yi in range(fl_y, fl_y + fl_h, d):
            half    = d // 2
            y_mid   = yi + half
            y_bot   = min(yi + d, fl_y + fl_h)
        ...
        # "BY" — строго по центру флага, сразу под ним
        by_x = fl_x + fl_w // 2
        by_y = fl_y + fl_h + 4
        draw.text((by_x, by_y), "BY", fill="#111111", font=fnt_by2, anchor="mt")
        ...
        # KZ: белый номер. Слева большой флаг + KZ. Вертикальный разделитель справа → регион.
        pw, ph = 530, 118
        px, py = cx - pw // 2, cy - ph // 2
        draw.rounded_rectangle([px+4, py+4, px+pw+4, py+ph+4], radius=8, fill="#b0b0b0")
        draw.rounded_rectangle([px, py, px+pw, py+ph], radius=8, fill="white",
        ...
        # ── Правая колонка с регионом (вертикальный разделитель, как у RU) ──────
        right_w = 76
        rdx = px + pw - right_w
        draw.line([(rdx, py + 6), (rdx, py + ph - 6)], fill="#111111", width=3)
        rcx = rdx + right_w // 2
        fnt_reg = ImageFont.truetype(FONT_BOLD, 44)
        draw.text((rcx, cy), region, fill="#111111", font=fnt_reg, anchor="mm")

        # ── Флаг Казахстана (большой, как смайлик 🇰🇿) ───────────────────────────
        zone_w = 104   # ширина зоны флага
        fl_w, fl_h = 86, 58   # флаг 3:2 пропорции
        ...
        # Левая орнаментальная полоска (кошкар-муиз) — 1/11 ширины, золотые ромбы
        orn_w = max(7, fl_w // 11)
        d_orn = orn_w
        ocx   = fl_x + orn_w // 2
        for yi in range(fl_y, fl_y + fl_h, d_orn):
            mid  = yi + d_orn // 2
            bot  = min(yi + d_orn, fl_y + fl_h)
            pts  = [(fl_x, mid), (ocx, yi), (fl_x + orn_w, mid), (ocx, bot)]
            draw.polygon(pts, fill=gold)

        # Солнце с 32 треугольными лучами (точно как на реальном флаге)
        content_w = fl_w - orn_w
        sun_cx = fl_x + orn_w + content_w // 2
        sun_cy = fl_y + int(fl_h * 0.36)
        sun_r  = max(6, fl_h // 6)
        ray_r  = sun_r + max(5, fl_h // 7)
        num_rays = 32
        for i in range(num_rays):
            a      = _math.radians(i * 360 / num_rays - 90)
            a_l    = _math.radians((i - 0.5) * 360 / num_rays - 90)
            a_r    = _math.radians((i + 0.5) * 360 / num_rays - 90)
            tip    = (int(sun_cx + ray_r  * _math.cos(a)),   int(sun_cy + ray_r  * _math.sin(a)))
            base_l = (int(sun_cx + sun_r  * _math.cos(a_l)), int(sun_cy + sun_r  * _math.sin(a_l)))
            base_r = (int(sun_cx + sun_r  * _math.cos(a_r)), int(sun_cy + sun_r  * _math.sin(a_r)))
            draw.polygon([base_l, tip, base_r], fill=gold)
        draw.ellipse([sun_cx-sun_r, sun_cy-sun_r, sun_cx+sun_r, sun_cy+sun_r], fill=gold)

        # Степной орёл — силуэт парящей птицы
        e_cx = sun_cx
        e_cy = fl_y + int(fl_h * 0.71)
        ws   = content_w // 2 - 4   # полуразмах крыла
        # Левое крыло
        draw.polygon([
            (e_cx - 2,  e_cy),
            (e_cx - ws//2, e_cy - int(fl_h * 0.11)),
            (e_cx - ws, e_cy - int(fl_h * 0.07)),
            (e_cx - ws + 2, e_cy + 1),
        ], fill=gold)
        # Правое крыло
        draw.polygon([
        ...
        # Тело-голова
        draw.ellipse([e_cx-4, e_cy-3, e_cx+4, e_cy+3], fill=gold)

        # "KZ" строго под флагом
        draw.text((fl_x + fl_w//2, fl_y + fl_h + 4), "KZ", fill="#111111",
                  font=fnt_kz, anchor="mt")

        # ── Основной номер — очень крупно, заполняет всё пространство ────────────
        c = chars.replace(" ", "")
        body     = f"{c[:3]} {c[3:]}" if len(c) == 6 else chars
        fnt_pl   = ImageFont.truetype(FONT_BOLD, 78)
        num_left = px + zone_w + 6
        num_cx   = num_left + (rdx - 6 - num_left) // 2
...
async def get_sticker_data(bot, msg, pack_type: str = "sticker"):
    ...
    Поддерживает: фото, PNG, WEBP, WEBM, TGS, готовые стикеры TG, GIF, анимированные стикеры.
    """
    ...
    # ── Готовый стикер из Telegram (статичный / анимированный / видео) ─────────
    if msg.sticker:
        s   = msg.sticker
        ...
        # Анимированный (.tgs) и видео (.webm) — возвращаем как есть
        return raw, fmt

    # ── Документ (PNG, WEBP, TGS, WEBM, GIF и т.д.) ──────────────────────────
    if msg.document:
        d    = msg.document
        ...
        if "gif" in mime or name.endswith(".gif"):
            # GIF → конвертируем в WEBM-видеостикер через PIL (берём первый кадр)
            try:
                raw = await dl(d.file_id)
                gif_img = Image.open(io.BytesIO(raw)).convert("RGBA")
    ...
    # ── GIF-анимация (animation в Telegram API) ───────────────────────────────
    if msg.animation:
...
async def mgmt_icon(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await query.edit_message_text(
        ...
        "Отправь стикер, фото или PNG-файл для использования как иконка:",
        parse_mode="HTML",
        reply_markup=InlineKeyboardMarkup([back_btn("◀️ Отмена", f"select_pack_{idx}")]),
    )
    return CHANGE_ICON
...
async def receive_icon(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    # Принимаем стикер, фото или документ (PNG/WEBP)
    raw_bytes = None
    icon_fmt  = StickerFormat.STATIC
    ...
    try:
        if msg.sticker:
            ...
            if hasattr(msg.sticker, "format"):
                icon_fmt = msg.sticker.format
        elif msg.photo:
            ...
            await msg.reply_text("❌ Отправь стикер, фото или PNG-файл для иконки.")
            return CHANGE_ICON
    except Exception as e:
    ...
    try:
        thumb = process_image_for_thumbnail(raw_bytes)
        thumb_buf = io.BytesIO(thumb)
        thumb_buf.seek(0)
        await context.bot.set_sticker_set_thumbnail(
            name=pack["name"],
            user_id=user_id,
            thumbnail=InputFile(thumb_buf, filename="thumb.png"),
            format=icon_fmt,
        )
    except TelegramError as e:
    ...
    conv = ConversationHandler(
        ...
        states={
            ...
            CHANGE_ICON: [
                CallbackQueryHandler(select_pack, pattern="^select_pack_\\d+$"),
                MessageHandler(
                    filters.Sticker.ALL | filters.PHOTO | filters.Document.IMAGE,
                    receive_icon,
                ),
            ],
            # ── Номерные знаки ──────────────────────────────────────────────

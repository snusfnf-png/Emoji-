    ...
    try:
        if mode == "single":
            ...
            if pack_created:
                ...
                kb_paint = [
                    [InlineKeyboardButton("🔗 Открыть пак", url=pack_link)],
                    [InlineKeyboardButton("💾 Сохранить", callback_data="paint_save_pack")],
...
async def send_main_menu(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    kb = [
        [InlineKeyboardButton("<tg-emoji emoji-id='5870528606328852614'>📁</tg-emoji>  Создать стикер-пак",      callback_data="create_sticker")],
        [InlineKeyboardButton("<tg-emoji emoji-id='5870764288364252592'>🙂</tg-emoji>  Создать эмодзи-пак",       callback_data="create_emoji")],
        [InlineKeyboardButton("🚗  Создать номерной знак",    callback_data="create_plate")],
        [InlineKeyboardButton("<tg-emoji emoji-id='5870528606328852614'>📁</tg-emoji>  Скопировать пак",          callback_data="copy_pack")],
        [InlineKeyboardButton("<tg-emoji emoji-id='6050679691004612757'>🖌</tg-emoji>  Покрасить стикер",         callback_data="paint_start")],
    ]
    text = (
...
async def _do_copy_pack(update_or_msg, context: ContextTypes.DEFAULT_TYPE,
    ...
    if creating_new:
        kb.append([InlineKeyboardButton("💾 Сохранить", callback_data="copy_save_pack")])

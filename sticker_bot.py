...
def recolor_webp(raw: bytes, hex_color: str) -> bytes:
    """
    Перекраска через luminosity-tint (soft-light + color overlay).
    Сохраняет структуру изображения, но тонирует цветом.
    Работает на фото-реалистичных, серых и чёрно-белых стикерах.
    Прозрачность (альфа-канал) полностью сохраняется.
    """
    h = hex_color.lstrip("#")
    tr, tg, tb = int(h[0:2], 16) / 255.0, int(h[2:4], 16) / 255.0, int(h[4:6], 16) / 255.0

    img = Image.open(io.BytesIO(raw)).convert("RGBA")
    arr = np.array(img, dtype=np.float32)

    alpha = arr[:, :, 3]  # сохраняем альфа

    # Воспринимаемая яркость
    lum = (0.299 * arr[:, :, 0] + 0.587 * arr[:, :, 1] + 0.114 * arr[:, :, 2]) / 255.0

    # Soft-light blend: сохраняет структуру светлых/тёмных областей
    def soft_light(lum_n: np.ndarray, c: float) -> np.ndarray:
        dark = 2.0 * lum_n * c + lum_n * lum_n * (1.0 - 2.0 * c)
        light = 2.0 * lum_n * (1.0 - c) + np.sqrt(np.maximum(lum_n, 0)) * (2.0 * c - 1.0)
        return np.where(lum_n < 0.5, dark, light)

    sl_r = soft_light(lum, tr)
    sl_g = soft_light(lum, tg)
    sl_b = soft_light(lum, tb)

    # Color overlay поверх soft-light с весом 0.45 — усиливает насыщенность цвета
    OVERLAY = 0.45
    final_r = sl_r * (1 - OVERLAY) + tr * lum * OVERLAY
    final_g = sl_g * (1 - OVERLAY) + tg * lum * OVERLAY
    final_b = sl_b * (1 - OVERLAY) + tb * lum * OVERLAY
...
async def paint_start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await query.edit_message_text(
        ...
        "<i>Поддерживаются статичные WebP, анимированные TGS и премиум-эмодзи паки.</i>",
        parse_mode="HTML",
...
async def paint_receive_sticker(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Пользователь прислал стикер — спрашиваем: один или весь пак."""
    ...
    if s.set_name:
        ...
        context.user_data.setdefault("paint_sticker", {})["mode"] = "single"
        return PAINT_COLOR
...
async def paint_receive_custom_emoji(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    Извлекаем file_id и имя пака из первого кастомного эмодзи,
    затем спрашиваем: красить только этот эмодзи или весь пак.
    """
    msg = update.message
    # Ищем custom_emoji entity
    ...
    kind = "анимированный TGS" if is_animated else "статичный WebP"

    if pack_name:
        # Есть пак — спрашиваем: один или весь пак (как для обычного стикера)
        await msg.reply_text(
            f"🖌 <b>Эмодзи получен</b> <i>({kind})</i>\n\n"
            f"📦 Пак: <code>{pack_name}</code>\n\n"
            f"Красить только этот эмодзи или весь пак?",
            parse_mode="HTML",
            reply_markup=_paint_kb_scope(),
        )
        return PAINT_SCOPE
    else:
        # Без пака — сразу к выбору цвета для одного эмодзи
        last = context.user_data.get("paint_last_color")
        await msg.reply_text(
            f"🖌 <b>Эмодзи получен</b> <i>({kind})</i>\n\nВыбери цвет:",
            parse_mode="HTML",
...
async def _paint_execute(update: Update, context: ContextTypes.DEFAULT_TYPE, hex_color: str):
    ...
    async def upload_and_make_sticker(raw: bytes, fmt: StickerFormat, emoji: str) -> InputSticker:
        ...
        try:
            return InputSticker(sticker=uploaded.file_id, emoji_list=[emoji], format=fmt_str)
        except TypeError:
            return InputSticker(sticker=uploaded.file_id, emoji_list=[emoji])
    ...
    try:
        if mode == "single":
            ...
            try:
                ...
                if any(x in dl_err_s for x in ("too big", "inaccessible", "invalid", "wrong")):
                    try:
            ...
            fmt = StickerFormat.ANIMATED if data["is_animated"] else StickerFormat.STATIC
            if data["is_animated"]:
                result = recolor_tgs(raw, hex_color)
            else:
                result = recolor_webp(raw, hex_color)
                # конвертируем webp → png для send_sticker
                img = Image.open(io.BytesIO(result)).convert("RGBA")
                buf_png = io.BytesIO()
                img.save(buf_png, "PNG")
                result = buf_png.getvalue()
                fmt = StickerFormat.STATIC

            fname = "sticker.tgs" if data["is_animated"] else "sticker.png"
            uploaded = await bot.upload_sticker_file(
                user_id=uid,
            ...
            if data["is_animated"]:
                # Анимированные TGS нельзя напрямую отправить через send_sticker —
                # создаём временный пак и отправляем стикер из него
                ...
                emoji_char = data.get("emoji") or "⭐"
                is_custom = data.get("is_custom_emoji", False)
                stype = StickerType.CUSTOM_EMOJI if is_custom else StickerType.REGULAR
                fmt_str = fmt.value if hasattr(fmt, "value") else str(fmt)

                # Собираем InputSticker
                try:
                    inp = InputSticker(sticker=uploaded.file_id, emoji_list=[emoji_char], format=fmt_str)
                except TypeError:
                    inp = InputSticker(sticker=uploaded.file_id, emoji_list=[emoji_char])

                # Создаём пак — один общий try/except
                pack_created_ok = False
                create_err = None
                for use_fmt in (True, False):  # сначала с sticker_format, потом без
                    try:
                        kwargs = dict(
                            user_id=uid, name=tmp_name,
                            title=f"Recolor [{hex_color}]",
                            stickers=[inp],
                            sticker_type=stype,
                        )
                        if use_fmt:
                            kwargs["sticker_format"] = fmt
                        await bot.create_new_sticker_set(**kwargs)
                        pack_created_ok = True
                        break
                    except Exception as e:
                        es = str(e).lower()
                        if "already" in es or "occupied" in es or "name is occupied" in es:
                            # пак уже есть — добавляем стикер в него
                            try:
                                await bot.add_sticker_to_set(user_id=uid, name=tmp_name, sticker=inp)
                                pack_created_ok = True
                            except Exception as add_e:
                                create_err = add_e
                            break
                        elif "invalid" in es and use_fmt:
                            # попробуем без sticker_format
                            create_err = e
                            continue
                        else:
                            create_err = e
                ...
                if not pack_created_ok:
                    raise Exception(f"Не удалось создать пак: {create_err}")
                ...
                try:
                    ...
                    pass
                await bot.send_sticker(msg.chat_id, uploaded.file_id)
            ...
            # Определяем формат по первому не-видео стикеру
            first = next((s for s in stickers if not s.is_video), None)
            if first is None:
                await status.edit_text("❌ Пак содержит только видео-стикеры — они не поддерживаются.", parse_mode="HTML")
                context.user_data.pop("paint_sticker", None)
                return
            ...
            for i, s in enumerate(stickers):
                ...
                try:
                    ...
                    fmt  = StickerFormat.ANIMATED if s.is_animated else StickerFormat.STATIC
                    if s.is_animated:
            ...
            if pack_created:
                ...
                await msg.reply_text(
                    ...
                    f"<i>Видео-стикеры (WEBM) пропущены.</i>",
                    parse_mode="HTML",

...
def generate_plate_image(country: str, chars: str, region: str, region_name: str) -> bytes:
    ...
    if country == "RU":
        ...
        # UA номер: [синяя полоса: флаг(синий+жёлтый) + "UA"] | [регион + номер]
        # Пример: [🇺🇦] АА 1234 ВВ
        pw, ph = 450, 118
        px, py = cx - pw // 2, cy - ph // 2
        ...
        # Синяя полоса слева
        strip_w = 58
        draw.rounded_rectangle([px, py, px+strip_w+10, py+ph], radius=8, fill="#003DA5")
        draw.rectangle([px+strip_w//2, py+3, px+strip_w, py+ph-3], fill="#003DA5")
        draw.line([(px+strip_w, py+4), (px+strip_w, py+ph-4)], fill="#111111", width=3)
        # Флаг Украины — две горизонтальные полосы в верхней части полосы
        ffw, ffh = 38, 26
        ffx = px + (strip_w - ffw) // 2
        ffy = py + 18
        draw.rectangle([ffx, ffy,        ffx+ffw, ffy+ffh//2], fill="#005BBB")   # синяя
        draw.rectangle([ffx, ffy+ffh//2, ffx+ffw, ffy+ffh],   fill="#FFD500")   # жёлтая
        draw.rectangle([ffx, ffy, ffx+ffw, ffy+ffh], outline="#aaaaaa", width=1, fill=None)
        # "UA" внизу полосы
        fnt_ua = ImageFont.truetype(FONT_BOLD, 17)
        draw.text((px + strip_w//2, py + ph - 14), "UA", fill="white", font=fnt_ua, anchor="mm")
        # Текст: регион (из region) + введённый номер (chars)
        # chars = "5525 РР", region = "АА" → показываем "АА 5525 РР"
        c = chars.strip().upper()
        body = f"{region} {c}" if region else c
        fnt_pl = ImageFont.truetype(FONT_BOLD, 56)
        content_cx = px + strip_w + (pw - strip_w) // 2
        draw.text((content_cx, cy), body, fill="#111111", font=fnt_pl, anchor="mm")

    elif country == "BY":
        # BY номер: [флаг BY компактно слева БЕЗ полосы] [1234 АР-6]
        # Флаг: горизонтальный — красная широкая + зелёная узкая + белый орнамент слева
        pw, ph = 470, 118
        px, py = cx - pw // 2, cy - ph // 2
        draw.rounded_rectangle([px+4, py+4, px+pw+4, py+ph+4], radius=8, fill="#b0b0b0")
        draw.rounded_rectangle([px, py, px+pw, py+ph], radius=8, fill="white",
                                outline="#111111", width=5)
        # Зона флага — компактно в левом углу
        zone_w = 68
        # Флаг BY: горизонтальный — красная (2/3 высоты) + зелёная (1/3 высоты)
        ffw2 = 50
        ffh2 = 70
        ffx2 = px + (zone_w - ffw2) // 2
        ffy2 = py + (ph - ffh2) // 2
        red_h = int(ffh2 * 0.67)
        grn_h = ffh2 - red_h
        draw.rectangle([ffx2,         ffy2,        ffx2+ffw2, ffy2+red_h], fill="#CF101A")
        draw.rectangle([ffx2,         ffy2+red_h,  ffx2+ffw2, ffy2+ffh2], fill="#009A44")
        # Белая полоска с орнаментом слева флага (вертикальная, ~1/6 ширины)
        orn_w = 9
        draw.rectangle([ffx2, ffy2, ffx2+orn_w, ffy2+ffh2], fill="white")
        # Красно-белый орнамент в белой полосе
        for yi in range(ffy2+2, ffy2+red_h-2, 9):
            draw.rectangle([ffx2+1, yi,   ffx2+4, yi+4], fill="#CF101A")
            draw.rectangle([ffx2+5, yi+4, ffx2+8, yi+8], fill="#CF101A")
        # "BY" по центру под флагом
        fnt_by2 = ImageFont.truetype(FONT_BOLD, 16)
        draw.text((ffx2 + ffw2//2, ffy2 + ffh2 + 9), "BY", fill="#111111", font=fnt_by2, anchor="mm")
        # Основной текст — "1234 АР-6"
        text_x = px + zone_w + 10
        c = chars.replace(" ", "")
        if len(c) >= 6:
        ...
        # KZ номер: [голубая полоса: флаг-смайлик + KZ] | [866 ZKK] | [15]
        # Флаг рисуем как реальный прямоугольник: голубой + солнце + орёл
        ...
        draw.rounded_rectangle([px, py, px+pw, py+ph], radius=8, fill="white",
                                outline="#111111", width=4)
        # Голубая полоса слева — плавный переход в белый
        strip_w = 72
        draw.rounded_rectangle([px, py, px+strip_w+10, py+ph], radius=8, fill="#00AFCA")
        draw.rectangle([px+strip_w//2, py+3, px+strip_w, py+ph-3], fill="#00AFCA")
        # Флаг Казахстана: голубой прямоугольник с солнцем (16 лучей) + орёл
        fkx = px + 8
        fky = py + 10
        fkw = strip_w - 16
        fkh = 58
        draw.rectangle([fkx, fky, fkx+fkw, fky+fkh], fill="#00AFCA")
        # Солнце — 16 лучей + жёлтый круг
        s_cx = fkx + fkw // 2
        s_cy = fky + 20
        for ai in range(0, 360, 22):
            a = _math.radians(ai)
            draw.line([
                (int(s_cx + 9*_math.cos(a)),  int(s_cy + 9*_math.sin(a))),
                (int(s_cx + 14*_math.cos(a)), int(s_cy + 14*_math.sin(a)))
            ], fill="#FFD700", width=2)
        draw.ellipse([s_cx-8, s_cy-8, s_cx+8, s_cy+8], fill="#FFD700")
        # Орёл — упрощённый: дуга + крылья
        e_y = fky + 44
        e_x = fkx + fkw // 2
        draw.arc([e_x-13, e_y-8, e_x+13, e_y+4], start=190, end=350, fill="#FFD700", width=2)
        draw.line([(e_x-13, e_y-4), (e_x-20, e_y-14)], fill="#FFD700", width=2)
        draw.line([(e_x+13, e_y-4), (e_x+20, e_y-14)], fill="#FFD700", width=2)
        # "KZ" под флагом — крупно и чётко
        fnt_kz = ImageFont.truetype(FONT_BOLD, 16)
        draw.text((fkx + fkw//2, fky + fkh + 11), "KZ", fill="white", font=fnt_kz, anchor="mm")
        # Правая панель с кодом региона — С разделителем
        right_w = 56
        rdx = px + pw - right_w
        draw.line([(rdx, py+4), (rdx, py+ph-4)], fill="#111111", width=3)
        fnt_reg = ImageFont.truetype(FONT_BOLD, 36)

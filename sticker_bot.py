...
def _generate_plate_emoji(country: str, chars: str, region: str) -> bytes:
    """
    Вертикальная компоновка номера для эмодзи-пака (100×100px).
    Структура (сверху вниз):
      - флаг страны (иконка)
      - код страны (RUS / UA / BY / KZ)
      - основные символы номера (крупно)
      - код региона (мелко)
    Всё на белом прямоугольнике с чёрной рамкой, прозрачный фон.
    Рисуем в 8× (800px), даунсемплируем до 100px — максимальная чёткость.
    """
    DRAW_SIZE = 800   # рисуем в 8× для идеального качества
    sc = DRAW_SIZE / 100  # = 8.0

    img  = Image.new("RGBA", (DRAW_SIZE, DRAW_SIZE), (0, 0, 0, 0))
    draw = ImageDraw.Draw(img)

    PAD   = int(18 * sc)   # отступ от края холста до рамки
    RX    = PAD
    RY    = PAD
    RW    = DRAW_SIZE - PAD * 2
    RH    = DRAW_SIZE - PAD * 2
    ...
    # Тень
    sh = int(4 * sc)
    draw.rounded_rectangle([RX+sh, RY+sh, RX+RW+sh, RY+RH+sh],
                            radius=RAD, fill=(0, 0, 0, 70))
    # Белый корпус
    draw.rounded_rectangle([RX, RY, RX+RW, RY+RH],
                            radius=RAD, fill="white",
                            outline="#111111", width=BRD)

    cx = RX + RW // 2

    # ── Зоны по высоте ──────────────────────────────────────────────────────
    inner_top = RY + BRD + int(4 * sc)
    inner_bot = RY + RH - BRD - int(4 * sc)
    inner_h   = inner_bot - inner_top

    # Зона флага: верхние 28%
    flag_zone_h = int(inner_h * 0.28)
    flag_cy     = inner_top + flag_zone_h // 2

    # Зона кода страны: следующие 12%
    country_zone_top = inner_top + flag_zone_h
    country_zone_h   = int(inner_h * 0.12)
    country_cy       = country_zone_top + country_zone_h // 2

    # Зона номера: следующие 42%
    num_zone_top = country_zone_top + country_zone_h
    num_zone_h   = int(inner_h * 0.42)
    num_cy       = num_zone_top + num_zone_h // 2

    # Зона региона: оставшиеся ~18%
    reg_zone_top = num_zone_top + num_zone_h
    reg_cy       = reg_zone_top + (inner_bot - reg_zone_top) // 2

    # ── Флаг ────────────────────────────────────────────────────────────────
    flag_h = int(flag_zone_h * 0.80)
    flag_w = int(flag_h * 1.6)  # стандартные пропорции флага

    FLAG_COLORS = {
        "RU": [("white", 1/3), ("#003DA5", 1/3), ("#CC0000", 1/3)],
        "UA": [("#005BBB", 1/2), ("#FFD500", 1/2)],
        "BY": [("#CF101A", 2/3), ("#007828", 1/3)],
        "KZ": [("#00AFCA", 1)],
    }
    ...
    if country == "KZ" and KZ_FLAG_PATH and os.path.exists(KZ_FLAG_PATH):
        try:
            kz_img = Image.open(KZ_FLAG_PATH).convert("RGBA")
            kz_ratio = flag_h / kz_img.height
            kz_w = int(kz_img.width * kz_ratio)
            kz_h = flag_h
            if kz_w > flag_w:
                kz_ratio = flag_w / kz_img.width
                kz_w = flag_w
                kz_h = int(kz_img.height * kz_ratio)
            kz_resized = kz_img.resize((kz_w, kz_h), Image.LANCZOS)
            kfx = cx - kz_w // 2
            kfy = flag_cy - kz_h // 2
            img.paste(kz_resized, (kfx, kfy), kz_resized)
        except Exception:
            draw.rectangle([fx, fy, fx+flag_w, fy+flag_h], fill="#00AFCA")
    else:
        stripes = FLAG_COLORS.get(country, [("white", 1)])
        cur_y = fy
        for color, frac in stripes:
            sh_px = max(1, int(flag_h * frac))
            end_y = min(fy + flag_h, cur_y + sh_px)
            if cur_y < end_y:
                draw.rectangle([fx, cur_y, fx + flag_w, end_y], fill=color)
            cur_y = end_y
        # Орнамент Беларуси
        if country == "BY":
            ow = max(3, flag_w // 9)
            draw.rectangle([fx, fy, fx+ow, fy+flag_h], fill="white")
        draw.rectangle([fx, fy, fx+flag_w, fy+flag_h],
                       outline="#444444", width=max(1, int(1.5*sc)))

    # ── Код страны ──────────────────────────────────────────────────────────
    country_labels = {"RU": "RUS", "UA": "UA", "BY": "BY", "KZ": "KZ"}
    c_label = country_labels.get(country, country)
    c_font_size = max(6, int(country_zone_h * 0.75))
    c_font = ImageFont.truetype(FONT_BOLD, c_font_size)
    draw.text((cx, country_cy), c_label, fill="#333333", font=c_font, anchor="mm")

    # ── Основной номер ───────────────────────────────────────────────────────
    # Форматируем номер по стране (та же логика что в горизонтальной версии)
    c_raw = chars.replace(" ", "").upper()
    if country == "RU":
        body = chars.upper()
    elif country == "KZ":
        body = f"{c_raw[:3]} {c_raw[3:]}" if len(c_raw) == 6 else chars.upper()
    elif country == "BY":
        if len(c_raw) >= 6:
            body = f"{c_raw[:4]} {c_raw[4:6]}"
        elif len(c_raw) >= 4:
            body = f"{c_raw[:4]} {c_raw[4:]}"
        else:
            body = chars.upper()
    else:
        if len(c_raw) == 6:
    ...
    # Подбираем размер шрифта чтобы номер влезал по ширине
    avail_w = RW - BRD * 2 - int(10 * sc)
    num_font_size = int(num_zone_h * 0.72)
    while num_font_size > int(8 * sc):
        try:
            nf = ImageFont.truetype(FONT_BOLD, num_font_size)
            bb = nf.getbbox(body)
            if (bb[2] - bb[0]) <= avail_w:
                break
        except Exception:
            break
        num_font_size = int(num_font_size * 0.92)

    num_font = ImageFont.truetype(FONT_BOLD, max(int(8*sc), num_font_size))
    draw.text((cx, num_cy), body, fill="#111111", font=num_font, anchor="mm")

    # ── Регион ───────────────────────────────────────────────────────────────
    reg_font_size = max(int(6*sc), int((inner_bot - reg_zone_top) * 0.60))
    reg_font = ImageFont.truetype(FONT_BOLD, reg_font_size)
    draw.text((cx, reg_cy), region, fill="#444444", font=reg_font, anchor="mm")

    # ── Даунсемплинг до 100×100 ──────────────────────────────────────────────

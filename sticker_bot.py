...
def recolor_webp(raw: bytes, hex_color: str) -> bytes:
    """
    Полная перекраска: яркость пикселя * целевой цвет + gamma-усиление.
    Белый (#FFFFFF) даёт реально белые/светлые результаты, не серые.
    ...
    # gamma 0.6: поднимает средние тона — белый реально белый, а не серый
    lum_g = np.power(np.maximum(lum, 0), 0.6)

    final_r = np.clip(tr * lum_g * 255, 0, 255).astype(np.uint8)
    final_g = np.clip(tg * lum_g * 255, 0, 255).astype(np.uint8)
    final_b = np.clip(tb * lum_g * 255, 0, 255).astype(np.uint8)
...
def _lottie_map_color(arr: list, tr: float, tg: float, tb: float) -> list:
    L = (0.299 * arr[0] + 0.587 * arr[1] + 0.114 * arr[2]) ** 0.6
    result = [round(min(tr * L, 1.0), 6), round(min(tg * L, 1.0), 6), round(min(tb * L, 1.0), 6)]
    if len(arr) >= 4:
        result.append(arr[3])
    return result

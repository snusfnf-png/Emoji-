...
async def copy_pack_receive_link(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    text = update.message.text.strip()
    # Принимаем форматы: t.me/addstickers/NAME, https://t.me/addstickers/NAME, просто NAME
    pack_name = text
    if "t.me/addstickers/" in text:
        pack_name = text.split("t.me/addstickers/")[-1].strip("/").strip()
    elif "t.me/" in text:
        pack_name = text.split("t.me/")[-1].strip("/").strip()
    ...
    for i, src_sticker in enumerate(stickers):
        ...
        for dl_attempt in range(4):
            try:
                ...
                err_s = str(dl_err)
                if "flood" in err_s.lower() or "retry" in err_s.lower():
                    wait = 5 * (dl_attempt + 1)
                    logger.info("copy_pack: flood on download, waiting %ds", wait)
                    await asyncio.sleep(wait)
                else:
                    logger.warning("copy_pack: download TG error sticker %d: %s", i, dl_err)
                    break
            except Exception as dl_err:
                logger.warning("copy_pack: download error sticker %d (attempt %d): %s",
                               i, dl_attempt, dl_err)
                await asyncio.sleep(2)

        if not raw:
            logger.warning("copy_pack: skipping sticker %d — download failed", i)
            errors += 1
            continue
        ...
        try:
            if target_type == "sticker":
                # Цель — обычный стикер-пак (512x512)
                ...
                # Цель — эмодзи-пак (custom_emoji, 100x100)
                if src_fmt == StickerFormat.STATIC:
            ...
            logger.warning("copy_pack: conversion error sticker %d: %s", i, conv_err)
            errors += 1
            continue
        ...
        stickers_buf_dummy = [{}] * added  # имитируем буфер (пустой = создать, не пустой = добавить)

        # ── Пушим в Telegram с ретраями ───────────────────────────────────────
        push_ok = False
        for push_attempt in range(4):
            err = await _push_sticker_to_tg(
            ...
            if "flood" in err_s.lower() or "retry" in err_s.lower() or "временно" in err_s.lower():
                ...
                logger.info("copy_pack: flood on push sticker %d, waiting %ds", i, wait)
                await asyncio.sleep(wait)
            elif "STICKERS_TOO_MUCH" in err_s or "максимальное" in err_s:
                logger.warning("copy_pack: pack full at sticker %d", i)
                await status_msg.edit_text(
                    f"⚠️ Пак заполнен (максимум 120 стикеров)!\n\n"
                    f"Скопировано: <b>{added}/{total}</b>",
                    parse_mode="HTML",
                )
                break
            else:
                logger.warning("copy_pack: push error sticker %d: %s", i, err)
                break  # не ретраим другие ошибки

        if push_ok:
            added += 1
            # Небольшая пауза чтобы не получить флуд от Telegram

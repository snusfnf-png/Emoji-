...
def generate_plate_image(country: str, chars: str, region: str, region_name: str) -> bytes:
    ...
    if country == "RU":
        ...
        # ── Флаг 🇰🇿 через NotoColorEmoji ────────────────────────────────────
        EMOJI_FONT = "/usr/share/fonts/truetype/noto/NotoColorEmoji.ttf"
        target_flag_h = 32   # желаемая высота флага на номере

        fnt_kz_label = ImageFont.truetype(FONT_BOLD, 14)
        kz_h = fnt_kz_label.getbbox("KZ")[3] - fnt_kz_label.getbbox("KZ")[1]
        # Рендерим emoji на временный холст при единственном доступном размере 109
        try:
            fnt_emoji = ImageFont.truetype(EMOJI_FONT, 109)
            tmp = Image.new("RGBA", (140, 140), (0, 0, 0, 0))
            ImageDraw.Draw(tmp).text((5, 5), "🇰🇿", font=fnt_emoji, embedded_color=True)
            bbox = tmp.getbbox()
            if bbox:
                cropped = tmp.crop(bbox)
                ratio   = target_flag_h / cropped.height
                resized = cropped.resize(
                    (max(1, int(cropped.width * ratio)), target_flag_h), Image.LANCZOS)
                flag_y = py + (ph - resized.height - 4 - kz_h) // 2

...
def _friendly_tg_error(err: str) -> str:
    e = str(err)
    if "STICKERSET_INVALID" in e or "invalid sticker" in e.lower():
    ...
    if "STICKER_PNG_NOPNG" in e or "wrong file type" in e.lower():
        return "Неверный формат файла. Отправь PNG, WEBP, WEBM или TGS."
    if "STICKER_GIF_NODOCUMENT" in e:
        return "GIF не поддерживается. Используй WEBM или TGS для анимации."
...
async def _finish_batch(update_or_msg, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if n == 0:
        # Ничего не добавлено — просим стикер заново
        msg = update_or_msg.message if hasattr(update_or_msg, "message") else update_or_msg
        await msg.reply_text(
            "⚠️ Не удалось добавить ни одного стикера. Попробуй отправить файл ещё раз 👇\n\n/cancel — отменить"
        )
    ...
    if added == n:
        text = f"🎉 <b>Пак создан!</b>\n<b>{title}</b>\n\nСтикеров: {n}\n\nДобавь ещё или сохрани 👇"
    else:
        text = (f"✅ Добавлено {added} {plural_sticker(added)}!\n"
                f"Всего в паке: {n} {plural_sticker(n)}\n\nПродолжай или сохрани 👇")
    msg = update_or_msg.message if hasattr(update_or_msg, "message") else update_or_msg
    await msg.reply_text(
...
def _extract_emoji(text: str) -> list:
    """Извлекает только настоящие эмодзи из текста."""
    import unicodedata
    result = []
    i = 0
    while i < len(text):
        cp = ord(text[i])
        # Основные блоки эмодзи
        if (0x1F300 <= cp <= 0x1FAFF  # Misc symbols, emoticons, etc.
                ...
                or 0x2614  <= cp <= 0x2615
                or 0x2648  <= cp <= 0x2653
                or cp in (0x267F, 0x2693, 0x26A1, 0x26AA, 0x26AB,
                          0x26BD, 0x26BE, 0x26C4, 0x26C5, 0x26CE,
                          0x26D4, 0x26EA, 0x26F2, 0x26F3, 0x26F5,
                          0x26FA, 0x26FD, 0x2702, 0x2705, 0x2708,
                          0x2709, 0x270A, 0x270B, 0x270C, 0x270F,
                          ...
                          0x2797, 0x27A1, 0x27B0, 0x27BF, 0x2934,
                          0x2935, 0x2B05, 0x2B06, 0x2B07, 0x2B1B,
                          0x2B1C, 0x2B50, 0x2B55, 0x3030, 0x303D,
                          0x3297, 0x3299)):
            # Собираем весь кластер (ZWJ последовательности, модификаторы)
            cluster = text[i]
            i += 1
            while i < len(text):
                ncp = ord(text[i])
                if (ncp == 0x200D or 0xFE00 <= ncp <= 0xFE0F
                        or 0x1F3FB <= ncp <= 0x1F3FF
                        or 0x1F1E0 <= ncp <= 0x1F1FF
                        or 0xE0020 <= ncp <= 0xE007F):
                    cluster += text[i]
                    i += 1
                    if i < len(text):
                        cluster += text[i]
...
async def receive_emoji_for_new(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if err:
        await update.message.reply_text(
            f"❌ {_friendly_tg_error(err)}\n\nПопробуй ещё раз или /cancel"
        )
        # Не двигаем done — даём пользователю повторить
        return WAITING_EMOJI
...
async def _push_sticker_to_tg(bot, user_id: int, pack_name: str, pack_type: str,
    ...
    if isinstance(data, bytes):
        ...
        buf = io.BytesIO(data)
        buf.seek(0)
        sticker_file = InputFile(buf, filename=fname)
    else:
        sticker_file = data

    input_sticker = InputSticker(sticker=sticker_file, emoji_list=emoji_list[:20], format=fmt)
    ...
    if not stickers_buf:
        ...
        try:
            ...
            logger.error("create_new_sticker_set error: %s", e)
        ...
        try:
            ...
            logger.info("Sticker added to set: %s", pack_name)
        except TelegramError as e:
            logger.error("add_sticker_to_set error: %s", e)
            # Если пак не существует — создаём заново
            if "STICKERSET_INVALID" in str(e) or "not found" in str(e).lower():
                logger.info("Pack not found, recreating: %s", pack_name)
                if isinstance(data, bytes):
                    buf2 = io.BytesIO(data)
                    buf2.seek(0)
                    sticker_file2 = InputFile(buf2, filename=fname)
                    input_sticker2 = InputSticker(sticker=sticker_file2, emoji_list=emoji_list[:20], format=fmt)
                else:
                    input_sticker2 = input_sticker
                try:
                    await bot.create_new_sticker_set(
                        user_id=user_id,
                        name=pack_name,
                        title=title,
                        stickers=[input_sticker2],
                        sticker_type=sticker_type,
                    )
                    logger.info("Pack recreated: %s", pack_name)
                    return None
                except TelegramError as e2:
                    logger.error("recreate error: %s", e2)
                    return _friendly_tg_error(str(e2))
            return _friendly_tg_error(str(e))
    return None
...
async def receive_add_emoji(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if data is None:
        await update.message.reply_text("❌ Данные файла потерялись. Отправь файл ещё раз.")
        return ADD_STICKER_FILE

    pack_type_cur = pack.get("type", "sticker")
    action_word2  = "эмодзи" if pack_type_cur == "emoji" else "стикер"
    await update.message.reply_text(f"⏳ Добавляю {action_word2}...")

    bot_username = (await context.bot.get_me()).username
    pack_name    = pack["name"]
    stickers_buf = [True] * pack.get("count", 1)   # непустой список → add_sticker_to_set

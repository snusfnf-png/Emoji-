...
class DB:
    """PostgreSQL хранилище паков.
    Каждый запрос использует новое соединение — надёжно и без проблем с таймаутами.
    """
    ...
    def save_packs(cls, user_id: int, packs: list):
        ...
        try:
            ...
            with conn.cursor() as cur:
                ...
                """, (user_id, json.dumps(packs, ensure_ascii=False)))
            conn.close()
            logger.info("DB save_packs: user=%s packs=%d", user_id, len(packs))
        except Exception as e:
...
async def _push_sticker_to_tg(bot, user_id: int, pack_name: str, pack_type: str,
    ...
    # InputSticker — в PTB 20.7 нет параметра format, он не нужен
    def make_sticker():
    ...
    async def _create_set(sticker):
        ...
        # В PTB 20.7 create_new_sticker_set не принимает sticker_format
        kwargs = dict(
            user_id=user_id,
            name=pack_name,
            title=title,

...
async def paint_start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await query.edit_message_text(
        ...
        "Скинь стикер или ссылку на пак:\n"
...
async def paint_receive_sticker(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    # Видео кастомные эмодзи — не поддерживаются
    if is_custom_emoji and s.is_video:
        pack_hint = (
        ...
        await update.message.reply_text(
            ...
            f"<i>Видео эмодзи не поддерживаются.{pack_hint}</i>",
            parse_mode="HTML",
...
async def _paint_execute(update: Update, context: ContextTypes.DEFAULT_TYPE, hex_color: str):
    ...
    try:
        if mode == "single":
            ...
            # Видео эмодзи-паки красить нельзя
            if is_emoji_pack and first.is_video:
                await status.edit_text(
                    "❌ <b>Этот пак нельзя покрасить.</b>\n\n"
                    "<i>Видео эмодзи-паки не поддерживаются. Работают статичные, анимированные и адаптивные эмодзи-паки.</i>",
                    parse_mode="HTML",
                    reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🏠 В меню", callback_data="begin")]]),
                )
                context.user_data.pop("paint_sticker", None)

...
def generate_plate_image(country: str, chars: str, region: str, region_name: str) -> bytes:
    ...
    if country == "RU":
        ...
        # KZ: белый номер. Слева флаг Казахстана (точно по фото).
        # Флаг: голубой (#00AFCA) фон, жёлтый орнамент-полоска слева,
        # солнце с 32 треугольными лучами, степной орёл под солнцем.
        # Справа — регион в отдельной колонке с разделителем.
        pw, ph = 530, 118
        px, py = cx - pw // 2, cy - ph // 2
        draw.rounded_rectangle([px+4, py+4, px+pw+4, py+ph+4], radius=8, fill="#b0b0b0")
        draw.rounded_rectangle([px, py, px+pw, py+ph], radius=8, fill="white",
        ...
        # ── Правая колонка — регион с разделителем ────────────────────────
        right_w = 72
        rdx = px + pw - right_w
        draw.line([(rdx, py+6), (rdx, py+ph-6)], fill="#111111", width=3)
        ...
        zone_w  = 108
        fl_w, fl_h = 90, 60
        fnt_kz  = ImageFont.truetype(FONT_BOLD, 17)
        kz_bbox = fnt_kz.getbbox("KZ")
        kz_h    = kz_bbox[3] - kz_bbox[1]
        fl_x    = px + (zone_w - fl_w) // 2
        fl_y    = py + (ph - fl_h - 4 - kz_h) // 2

        # Голубой фон
        draw.rectangle([fl_x, fl_y, fl_x+fl_w, fl_y+fl_h], fill=blue)

        # ── Орнамент "кошкар-муиз" — вертикальная полоска слева ──────────
        # По фото: жёлтые ромбические узоры в 1 колонке вдоль левого края
        orn_w = max(8, fl_w // 10)
        step  = orn_w + 1
        ocx_o = fl_x + orn_w // 2
        # Чередующиеся ромбы — как традиционный казахский орнамент
        for yi in range(fl_y + 1, fl_y + fl_h - 1, step):
            bot = min(yi + step, fl_y + fl_h - 1)
            mid = (yi + bot) // 2
            draw.polygon([
                (fl_x + 1,      mid),
                (ocx_o,         yi),
        ...
        # Маленькие ромбики между большими
        for yi in range(fl_y + step//2, fl_y + fl_h - step//2, step):
            s2 = max(2, step // 4)
            draw.polygon([
                (ocx_o,     yi - s2),
                (ocx_o+s2,  yi),
        ...
        # ── Солнце с 32 треугольными лучами ──────────────────────────────
        cont_w = fl_w - orn_w
        sun_cx = fl_x + orn_w + int(cont_w * 0.55)
        sun_cy = fl_y + int(fl_h * 0.37)
        sun_r  = max(7, fl_h // 6)
        ray_r  = sun_r + max(6, fl_h // 8)
        for i in range(32):
            a   = _math.radians(i * 360/32 - 90)
            al  = _math.radians((i-0.55)*360/32 - 90)
            ar  = _math.radians((i+0.55)*360/32 - 90)
            tip = (int(sun_cx+ray_r*_math.cos(a)),  int(sun_cy+ray_r*_math.sin(a)))
            bl  = (int(sun_cx+sun_r*_math.cos(al)), int(sun_cy+sun_r*_math.sin(al)))
            br  = (int(sun_cx+sun_r*_math.cos(ar)), int(sun_cy+sun_r*_math.sin(ar)))
            draw.polygon([bl, tip, br], fill=gold)
        draw.ellipse([sun_cx-sun_r, sun_cy-sun_r, sun_cx+sun_r, sun_cy+sun_r], fill=gold)

        # ── Степной орёл — парящий под солнцем ───────────────────────────
        e_cx = sun_cx
        e_cy = fl_y + int(fl_h * 0.73)
        ws   = cont_w // 2 - 2
        # Левое крыло (изогнутое вниз как у степного орла)
        draw.polygon([
            (e_cx,         e_cy),
            (e_cx - ws//3, e_cy - int(fl_h*0.10)),
            (e_cx - ws,    e_cy - int(fl_h*0.06)),
            (e_cx - ws,    e_cy + int(fl_h*0.03)),
            (e_cx - ws//2, e_cy + int(fl_h*0.04)),
        ], fill=gold)
        # Правое крыло
        draw.polygon([
            (e_cx,         e_cy),
        ...
        # Голова + тело
        draw.ellipse([e_cx-3, e_cy-5, e_cx+3, e_cy+1], fill=gold)

        # "KZ" под флагом
        draw.text((fl_x + fl_w//2, fl_y + fl_h + 4), "KZ", fill="#111111",
                  font=fnt_kz, anchor="mt")

        # ── Основной номер ─────────────────────────────────────────────────
        c    = chars.replace(" ", "")
        body = f"{c[:3]} {c[3:]}" if len(c) == 6 else chars
        fnt_pl  = ImageFont.truetype(FONT_BOLD, 76)
        num_left = px + zone_w + 4
        num_cx   = num_left + (rdx - 4 - num_left) // 2

...
def recolor_webm(raw: bytes, hex_color: str) -> bytes | None:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        with open(in_path, "wb") as f:
            f.write(raw)
        # Применяем colorchannelmixer + hue overlay для перекраски
        # Используем colorize через hue/saturation или colorchannelmixer
        color_filter = (
            f"colorchannelmixer="
            f"rr={r/255:.4f}:rg=0:rb=0:"
            f"gr=0:gg={g/255:.4f}:gb=0:"
            f"br=0:bg=0:bb={b/255:.4f},"
            f"hue=s=2.0"
        )
        cmd = [
            ffmpeg, "-y", "-i", in_path,
...
async def paint_start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await query.edit_message_text(
        ...
        "<i>Поддерживаются все типы: WebP, TGS, WEBM и премиум-эмодзи паки.</i>",
        parse_mode="HTML",
    ...
    try:
        if mode == "single":
            ...
            try:
                ...
                if raw is None:
                    if is_custom:
                        # Кастомные эмодзи Telegram не даёт скачивать — сообщаем об этом
                        try:
                            await status.delete()
                        except Exception:
                            pass
                        await msg.reply_text(
                            "⚠️ <b>Кастомные эмодзи нельзя скачать через API Telegram.</b>\n\n"
                            "Отправь ссылку на эмодзи-пак для покраски целого пака:\n"
                            "<code>t.me/addemoji/ИМЯ_ПАКА</code>",
                            parse_mode="HTML",
            ...
            for _ec in _emoji_candidates:
                try:
                    ...
                    # Проверяем что create_set тоже принимает этот эмодзи
                    try:
            ...
            if is_custom:
                # Кастомные эмодзи нельзя отправить через send_sticker —
                # Telegram запрещает это. Отправляем ссылку на пак.
                pack_link = f"https://t.me/addemoji/{tmp_name}"
                await msg.reply_text(
                    f"✅ <b>Готово!</b> Цвет: <code>{hex_color}</code>\n\n"
                    f"📦 <b>Перекрашенный эмодзи:</b> <a href=\"{pack_link}\">{tmp_name}</a>\n\n"
                    f"<i>Кастомные эмодзи нельзя отправить напрямую — открой пак по ссылке.</i>",
                    parse_mode="HTML",

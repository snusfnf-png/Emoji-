...
def generate_plate_image(country: str, chars: str, region: str, region_name: str) -> bytes:
    ...
    if country == "RU":
        ...
        # KZ: точная копия игры cardrop.
        # Структура: [флаг 🇰🇿 + KZ слева] [ЦЦЦ ЛЛЛ по центру] [регион справа с разделителем]
        # Флаг рендерится как цветной PIL-прямоугольник (emoji KZ: голубой + золото)
        pw, ph = 520, 118
        px, py = cx - pw // 2, cy - ph // 2
        # Тень
        ...
        SW  = 72   # ширина левой колонки с флагом
        RW  = 70   # ширина правой колонки с регионом
        col_cx = px + SW // 2   # центр левой колонки

        # ── Флаг KZ: точно как в игре — emoji через PIL рисование ─────────────
        # Голубой прямоугольник с золотым орнаментом, солнцем и орлом
        import math as _m
        gold = "#FFD700"
        kz_blue = "#00AFCA"

        # Размер флага внутри колонки
        fl_w = 46
        fl_h = 30
        fl_x = col_cx - fl_w // 2
        fl_y = py + (ph // 2) - fl_h - 4   # сдвинут чуть вверх чтобы влезла надпись KZ

        # Голубой фон флага
        draw.rectangle([fl_x, fl_y, fl_x + fl_w, fl_y + fl_h], fill=kz_blue)

        # Орнамент слева — золотые ромбы (как на реальном флаге)
        orn = max(5, fl_w // 9)
        ocx = fl_x + orn // 2
        step_o = orn + 1
        for yi in range(fl_y + 1, fl_y + fl_h - 1, step_o):
            bot = min(yi + step_o, fl_y + fl_h - 1)
            mid = (yi + bot) // 2
            draw.polygon([(fl_x+1, mid), (ocx, yi), (fl_x+orn-1, mid), (ocx, bot)], fill=gold)

        # Солнце с 32 лучами
        cw    = fl_w - orn
        s_cx  = fl_x + orn + int(cw * 0.55)
        s_cy  = fl_y + int(fl_h * 0.38)
        s_r   = max(4, fl_h // 6)
        r_r   = s_r + max(3, fl_h // 9)
        for i in range(32):
            a  = _m.radians(i * 360/32 - 90)
            al = _m.radians((i-0.55)*360/32 - 90)
            ar = _m.radians((i+0.55)*360/32 - 90)
            draw.polygon([
                (int(s_cx+s_r*_m.cos(al)), int(s_cy+s_r*_m.sin(al))),
                (int(s_cx+r_r*_m.cos(a)),  int(s_cy+r_r*_m.sin(a))),
                (int(s_cx+s_r*_m.cos(ar)), int(s_cy+s_r*_m.sin(ar))),
            ], fill=gold)
        draw.ellipse([s_cx-s_r, s_cy-s_r, s_cx+s_r, s_cy+s_r], fill=gold)

        # Орёл
        e_cx = s_cx
        e_cy = fl_y + int(fl_h * 0.74)
        ws   = cw // 2 - 2
        draw.polygon([(e_cx,0+e_cy),(e_cx-ws//3,e_cy-int(fl_h*.10)),(e_cx-ws,e_cy-int(fl_h*.06)),(e_cx-ws,e_cy+1)], fill=gold)
        draw.polygon([(e_cx,0+e_cy),(e_cx+ws//3,e_cy-int(fl_h*.10)),(e_cx+ws,e_cy-int(fl_h*.06)),(e_cx+ws,e_cy+1)], fill=gold)
        draw.ellipse([e_cx-2, e_cy-4, e_cx+2, e_cy], fill=gold)

        # "KZ" под флагом — точно по центру колонки
        fnt_kz = ImageFont.truetype(FONT_BOLD, 14)
        draw.text((col_cx, fl_y + fl_h + 3), "KZ", fill="#111111", font=fnt_kz, anchor="mt")

        # ── Правый разделитель + регион (точно как в игре: светло-серая линия) ────
        rdx = px + pw - RW
        ...
        # Центр между концом флага-колонки и началом региона

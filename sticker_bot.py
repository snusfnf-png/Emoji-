...
def generate_plate_image(country: str, chars: str, region: str, region_name: str) -> bytes:
    ...
    if country == "RU":
        ...
        rcx = rdx + right_w // 2
        # 1. Region code — large, top of right panel
        fnt_r = ImageFont.truetype(FONT_BOLD, 46)
        draw.text((rcx, py + 38), region, fill="#111111", font=fnt_r, anchor="mm")
        # 2. RUS text then flag on ONE line: "RUS 🏳"
        fw, fh = 22, 15
        ...
        fx = tx + rus_w + gap
        fy = row_y - fh // 2
        draw.text((tx, row_y), "RUS", fill="#111111", font=fnt_rus, anchor="lm")
...
async def _push_sticker_to_tg(bot, user_id: int, pack_name: str, pack_type: str,
    ...
    sticker_file  = data if not isinstance(data, bytes) else io.BytesIO(data)

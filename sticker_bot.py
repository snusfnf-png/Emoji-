...
async def _push_sticker_to_tg(bot, user_id: int, pack_name: str, pack_type: str,
    ...
    def make_input_sticker():
        if isinstance(data, bytes):
            ...
            sticker_file = data
        return InputSticker(sticker=sticker_file, emoji_list=emoji_list[:20], format=fmt)
    ...
    if not stickers_buf:
        ...
        try:
            ...
            if "already occupied" in err_str.lower() or "name is already" in err_str.lower():
                logger.info("Pack already exists, adding sticker instead: %s", pack_name)
                try:
                    await bot.add_sticker_to_set(
                        user_id=user_id,
                        name=pack_name,
                        sticker=make_input_sticker(),
                    )
                    return None
                except TelegramError as e2:
                    logger.error("add_sticker_to_set (fallback) error: %s", e2)

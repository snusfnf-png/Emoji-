...
def generate_plate_image(country: str, chars: str, region: str, region_name: str) -> bytes:
    ...
    if country == "RU":
        ...
        # BY: белорусский номерной знак
        # Флаг: красная (2/3 высоты) + зелёная (1/3 высоты)
        # Орнамент: белая вертикальная полоса ~1/9 ширины флага с КРАСНЫМИ/ЗЕЛЁНЫМИ ромбами
        pw, ph = 490, 118
        px, py = cx - pw // 2, cy - ph // 2
        draw.rounded_rectangle([px+4, py+4, px+pw+4, py+ph+4], radius=8, fill="#b0b0b0")
        draw.rounded_rectangle([px, py, px+pw, py+ph], radius=8, fill="white",
        ...
        # Белая орнаментальная полоска — 1/9 ширины
        orn_w = max(7, fl_w // 9)
        draw.rectangle([fl_x, fl_y, fl_x+orn_w, fl_y+fl_h], fill="white")

        # Орнамент: традиционные белорусские ромбы (вышивка)
        # На красной части — красные ромбы на белом фоне
        # На зелёной части — зелёные ромбы на белом фоне
        step = max(5, orn_w + 1)
        ocx  = fl_x + orn_w // 2
        ...
        # KZ: смайлик флага 🇰🇿 (NotoColorEmoji, размер 109→resize) + "KZ" под ним
        pw, ph = 520, 118
        px, py = cx - pw // 2, cy - ph // 2
        ...
        # ── Флаг 🇰🇿 — PNG из noto-emoji ────────────────────────────────────
        target_flag_h = 56

        fnt_kz_label = ImageFont.truetype(FONT_BOLD, 18)
        ...
        if KZ_FLAG_PATH and os.path.exists(KZ_FLAG_PATH):
            try:
                ...
                # Масштабируем по высоте, но не шире чем зона (SW - отступы)
                max_flag_w = SW - 16
        ...
        if not flag_drawn:
            # Fallback — голубой прямоугольник
            fl_w, fl_h = 60, 40
            total_h = fl_h + 3 + kz_h
            flag_y  = py + (ph - total_h) // 2
            fl_x    = col_cx - fl_w // 2
        ...
        fnt_pl = ImageFont.truetype(FONT_BOLD, 78)
...
def generate_plate_image_transparent(country: str, chars: str, region: str,
                                      region_name: str, target_size: int = 512) -> bytes:
    """
    Рисует только номерной знак на прозрачном фоне — без серого фона и ватермарок.
    Знак занимает ~90% target_size по ширине, выровнен по центру.
    Возвращает PNG с альфа-каналом (RGBA).
    """
    # Определяем пропорции знака для каждой страны
    PLATE_RATIOS = {
        "RU": (365, 118),
        "UA": (450, 118),
        "BY": (490, 118),
    ...
    pw_base, ph_base = PLATE_RATIOS.get(country, (450, 118))

    # Масштабируем так, чтобы ширина знака = ~90% target_size
    scale = (target_size * 0.90) / pw_base
    pw = int(pw_base * scale)
    ph = int(ph_base * scale)

    # Холст чуть больше знака (небольшой отступ для тени)
    PAD = int(8 * scale)
    W = pw + PAD * 2
    H = ph + PAD * 2
    ...
    # Шрифты масштабируем
    def fnt(size):
        return ImageFont.truetype(FONT_BOLD, max(8, int(size * scale)))
    def fnt_r(size):
        return ImageFont.truetype(FONT_REG, max(8, int(size * scale)))

    if country == "RU":
        # Тень
        draw.rounded_rectangle([px+PAD//2, py+PAD//2, px+pw+PAD//2, py+ph+PAD//2],
                                radius=int(10*scale), fill=(0,0,0,60))
        # Корпус
        draw.rounded_rectangle([px, py, px+pw, py+ph],
                                radius=int(10*scale), fill="white",
                                outline="#111111", width=max(2, int(4*scale)))
        right_w = int(92 * scale)
        rdx = px + pw - right_w
        draw.line([(rdx, py+int(6*scale)), (rdx, py+ph-int(6*scale))],
                  fill="#111111", width=max(1, int(3*scale)))
        # Основные символы
        draw.text((px + (pw - right_w) // 2, cy), chars,
                  fill="#111111", font=fnt(66), anchor="mm")
        # Правая панель
        rcx = rdx + right_w // 2
        rpt = py + int(6*scale)
        rpb = py + ph - int(6*scale)
        rph = rpb - rpt
        region_cy = rpt + int(rph * 0.42)
        draw.text((rcx, region_cy), region, fill="#111111", font=fnt(44), anchor="mm")
        # RUS + флаг
        fw, fh = int(20*scale), int(14*scale)
        fnt_rus = fnt_r(12)
        rus_w = int(fnt_rus.getlength("RUS"))
        gap = int(3*scale)
        total_w = rus_w + gap + fw
        ...
        draw.rounded_rectangle([px+PAD//2, py+PAD//2, px+pw+PAD//2, py+ph+PAD//2],
                                radius=int(8*scale), fill=(0,0,0,60))
        draw.rounded_rectangle([px, py, px+pw, py+ph],
                                radius=int(8*scale), fill="white",
                                outline="#111111", width=max(2, int(4*scale)))
        strip_w = int(58 * scale)
        draw.rounded_rectangle([px+2, py+2, px+strip_w, py+ph-2],
                                radius=int(7*scale), fill="#003DA5")
        draw.line([(px+strip_w, py+int(4*scale)), (px+strip_w, py+ph-int(4*scale))],
                  fill="#111111", width=max(1, int(3*scale)))
        ffw, ffh = int(36*scale), int(24*scale)
        ffx = px + (strip_w - ffw) // 2
        ffy = py + int(18*scale)
        draw.rectangle([ffx, ffy,        ffx+ffw, ffy+ffh//2], fill="#005BBB")
        draw.rectangle([ffx, ffy+ffh//2, ffx+ffw, ffy+ffh],   fill="#FFD500")
        draw.text((px + strip_w//2, py + ph - int(14*scale)), "UA",
                  fill="white", font=fnt(17), anchor="mm")
        c = chars.strip().upper().replace(" ", "")
        if len(c) == 6:
            body = f"{region} {c[:4]} {c[4:]}" if region else f"{c[:4]} {c[4:]}"
        elif len(c) == 4:
            ...
            body = f"{region} {c}" if region else c
        content_cx = px + strip_w + (pw - strip_w) // 2
        draw.text((content_cx, cy), body, fill="#111111", font=fnt(62), anchor="mm")
        ...
        draw.rounded_rectangle([px, py, px+pw, py+ph],
                                radius=int(8*scale), fill="white",
                                outline="#111111", width=max(2, int(5*scale)))
        zone_w = int(90 * scale)
        fl_w   = int(72 * scale)
        fl_h   = int(48 * scale)
        fnt_by2 = fnt(16)
        ...
        draw.text((fl_x + fl_w//2, fl_y + fl_h + int(4*scale)), "BY",
                  fill="#111111", font=fnt_by2, anchor="mt")
        text_x = px + zone_w + int(6*scale)
        c = chars.replace(" ", "").upper()
        if len(c) >= 6:
            body = f"{c[:4]} {c[4:6]}-{region}"
        elif len(c) >= 4:
            ...
            body = f"{chars}-{region}"
        content_cx = text_x + (px + pw - int(8*scale) - text_x) // 2
        draw.text((content_cx, cy), body, fill="#111111", font=fnt(68), anchor="mm")
        ...
        draw.rounded_rectangle([px, py, px+pw, py+ph],
                                radius=int(10*scale), fill="white",
                                outline="#1a1a1a", width=max(2, int(5*scale)))
        SW = int(96 * scale)
        RW = int(68 * scale)
        col_cx = px + SW // 2
        target_flag_h = int(56 * scale)
        fnt_kz_label = fnt(18)
        kz_bbox = fnt_kz_label.getbbox("KZ")
        kz_h = kz_bbox[3] - kz_bbox[1]
        flag_drawn = False
        if KZ_FLAG_PATH and os.path.exists(KZ_FLAG_PATH):
            try:
                ...
                new_h = max(1, int(flag_img.height * ratio_h))
                resized  = flag_img.resize((new_w, new_h), Image.LANCZOS)
                total_h  = resized.height + int(3*scale) + kz_h
                flag_y   = py + (ph - total_h) // 2
                flag_x   = col_cx - resized.width // 2
                img.paste(resized, (flag_x, flag_y), resized)
                flag_bottom = flag_y + resized.height
                ...
                pass
        if not flag_drawn:
            fl_w, fl_h = int(60*scale), int(40*scale)
            total_h = fl_h + int(3*scale) + kz_h
            flag_y  = py + (ph - total_h) // 2
            fl_x    = col_cx - fl_w // 2
            draw.rectangle([fl_x, flag_y, fl_x+fl_w, flag_y+fl_h], fill="#00AFCA")
            flag_bottom = flag_y + fl_h
        draw.text((col_cx, flag_bottom + int(3*scale)), "KZ",
                  fill="#111111", font=fnt_kz_label, anchor="mt")
        rdx = px + pw - RW
        draw.line([(rdx, py+int(10*scale)), (rdx, py+ph-int(10*scale))],
                  fill="#bbbbbb", width=max(1, int(2*scale)))
        draw.text((rdx + RW // 2, cy), region, fill="#111111", font=fnt(42), anchor="mm")
        c    = chars.replace(" ", "")
        body = f"{c[:3]} {c[3:]}" if len(c) == 6 else chars
        num_cx = px + SW + (rdx - px - SW) // 2
        draw.text((num_cx, cy), body, fill="#111111", font=fnt(78), anchor="mm")

    # Финальный холст строго target_size × target_size
    out = Image.new("RGBA", (target_size, target_size), (0, 0, 0, 0))
    # Вписываем знак с сохранением пропорций
    ratio = min(target_size / W, target_size / H)
    nw = max(1, int(W * ratio))
    nh = max(1, int(H * ratio))
    resized_plate = img.resize((nw, nh), Image.LANCZOS)
    off_x = (target_size - nw) // 2
    off_y = (target_size - nh) // 2
    out.paste(resized_plate, (off_x, off_y), resized_plate)
...
async def get_sticker_data(bot, msg, pack_type: str = "sticker"):
    """Извлекает данные стикера из сообщения.
    Поддерживает: фото, PNG, WEBP, WEBM, TGS, стикеры TG, видео, GIF.
    """
    ...
    def safe_process_image(raw: bytes, pt: str) -> bytes:
        """Обрабатывает изображение, проверяя что это действительно изображение."""
        try:
            Image.open(io.BytesIO(raw)).verify()  # проверяем что PIL может открыть
        except Exception:
            raise ValueError("Файл не является изображением")
        return process_image_for_sticker(raw, pt)
    ...
    if msg.sticker:
        ...
        if fmt == StickerFormat.STATIC:
            return safe_process_image(raw, pack_type), fmt
        return raw, fmt
    ...
    if msg.document:
        ...
        if "gif" in mime or name.endswith(".gif"):
            ...
            try:
                ...
                buf = io.BytesIO()
                frame.save(buf, "PNG")
                return safe_process_image(buf.getvalue(), pack_type), StickerFormat.STATIC
            except Exception:
                return None, None
        # MP4 / MOV / видео-документ → как VIDEO-стикер
        if "video" in mime or any(name.endswith(x) for x in (".mp4", ".mov", ".avi")):
            return await dl(d.file_id), StickerFormat.VIDEO
        # PNG, WEBP, JPEG и т.д. — изображения
        raw = await dl(d.file_id)
        try:
            return safe_process_image(raw, pack_type), StickerFormat.STATIC
        except Exception:
            # Если не изображение — пробуем как видео
            return raw, StickerFormat.VIDEO
    ...
    if msg.video:
        # Видео → видео-стикер. НЕ пропускаем через PIL!
        return await dl(msg.video.file_id), StickerFormat.VIDEO

    # ── GIF-анимация ──────────────────────────────────────────────────────────
    if msg.animation:
        # Анимация → видео-стикер
...
async def _prompt_first_sticker(update: Update, suffix: str, context=None) -> int:
    ...
    if is_adaptive:
        ...
        formats_text = "<i>Поддерживаются: фото, PNG, WEBP, WEBM, TGS, готовые стикеры/эмодзи TG, видео, GIF.</i>"
    text = (
...
async def receive_sticker_file(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        if data is None:
            ...
            return ADDING_STICKER
        # Для адаптивного пака — только PNG/WEBP (STATIC) или WEBM (VIDEO)
...
async def _push_sticker_to_tg(bot, user_id: int, pack_name: str, pack_type: str,
    ...
    async def _create_set(sticker):
        # needs_repainting=True — обязательный флаг для адаптивного пака
        # (эмодзи меняют цвет под тему TG, можно ставить на фон профиля/сообщений)
        is_adaptive = (pack_type == "emoji_adaptive")
...
async def receive_add_emoji(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        add_label = "➕ Добавить ещё эмодзи" if is_emoji_pack else "➕ Добавить ещё стикер"
        await update.message.reply_text(
...
async def mgmt_delete(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        await query.edit_message_text(
            f"❌ Не удалось загрузить пак: {_friendly_tg_error(str(e))}",
            reply_markup=InlineKeyboardMarkup([back_btn("◀️ Назад", f"select_pack_{idx}")]),
        )
        return PACK_SELECTED
    if not tg_pack.stickers:
        await query.edit_message_text(
            f"В паке нет {item_label}.",
            reply_markup=InlineKeyboardMarkup([back_btn("◀️ Назад", f"select_pack_{idx}")]),
        )
        return PACK_SELECTED
    kb = []
    for i, s in enumerate(tg_pack.stickers[:50]):
        label = s.emoji or f"#{i+1}"
        kb.append([InlineKeyboardButton(f"{label} #{i+1}", callback_data=f"del_sticker_{s.file_unique_id}")])
    kb.append(back_btn("◀️ Отмена", f"select_pack_{idx}"))
    await query.edit_message_text(
        f"🗑 Выбери {item_label} для удаления:",
        reply_markup=InlineKeyboardMarkup(kb),
    )
...
async def confirm_delete_sticker(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        await query.edit_message_text(
            f"❌ Ошибка: {_friendly_tg_error(str(e))}",
            reply_markup=InlineKeyboardMarkup([back_btn("◀️ Назад", f"select_pack_{idx}")]),
        )
        return PACK_SELECTED
    ...
    if not target:
        await query.edit_message_text(
            "❌ Элемент не найден. Возможно уже удалён.",
            reply_markup=InlineKeyboardMarkup([back_btn("◀️ Назад", f"select_pack_{idx}")]),
        )
        return PACK_SELECTED
    try:
        await context.bot.delete_sticker_from_set(target.file_id)
        logger.info("Deleted sticker %s from %s", file_unique_id, pack["name"])
    except TelegramError as e:
        await query.edit_message_text(
            f"❌ Не удалось удалить: {_friendly_tg_error(str(e))}",
            reply_markup=InlineKeyboardMarkup([
                back_btn("◀️ Назад к списку", f"select_pack_{idx}"),
            ]),
        )
        return DELETE_STICKER
...
async def mgmt_icon(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    url2     = pack_url(pack2["name"])
    await query.edit_message_text(
        "🖼 <b>Сменить иконку пака</b>\n\n"
        "Открой пак по ссылке, найди нужный стикер/эмодзи и <b>перешли его сюда</b>:",
        parse_mode="HTML",
        reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton("🔗 Открыть пак", url=url2)],
...
async def receive_icon(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if not msg.sticker:
        await msg.reply_text(
            "❌ Нужен стикер/эмодзи из этого пака.\n"
            "Открой пак, нажми на любой стикер и отправь его сюда."
        )
        return CHANGE_ICON
    ...
    try:
        ...
        # Для статичного стикера — конвертируем в миниатюру 100×100
        if icon_fmt == StickerFormat.STATIC:
            thumb_data = process_image_for_thumbnail(raw)
            thumb_buf  = io.BytesIO(thumb_data)
            thumb_buf.seek(0)
            thumb_file = InputFile(thumb_buf, filename="thumb.png")
        else:
            # Для анимированных/видео — используем оригинал
            thumb_buf  = io.BytesIO(raw)
            thumb_buf.seek(0)
            thumb_file = InputFile(thumb_buf, filename="thumb.webm" if icon_fmt == StickerFormat.VIDEO else "thumb.tgs")
        ...
        logger.info("Icon set for pack %s", pack["name"])
    except TelegramError as e:
        err = _friendly_tg_error(str(e))
        await msg.reply_text(
            f"❌ Не удалось установить иконку: {err}\n\n"
            "Убедись что стикер именно из этого пака.",
            reply_markup=InlineKeyboardMarkup([back_btn("◀️ Назад", f"select_pack_{idx}")]),
        )
        return CHANGE_ICON
    except Exception as e:
...
async def plate_receive_input(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        kb = [
            [InlineKeyboardButton("🖼 В стикер-пак",       callback_data="plate_save_sticker"),
             InlineKeyboardButton("✨ В эмодзи-пак",        callback_data="plate_save_emoji")],
...
def _save_plate_pack(context, user_id: int, pack_type: str, pack: dict):
    """Сохраняет пак номерных знаков и добавляет в общий список паков."""
    key = f"plate_{pack_type}_pack"
    context.bot_data.setdefault(str(user_id), {})[key] = pack
    packs = get_packs(context, user_id)
    # Не дублируем
    if not any(p["name"] == pack["name"] for p in packs):
        packs.append(pack)


async def _plate_add_to_pack(bot, user_id: int, img_bytes: bytes,
                              pack: dict, emoji: str) -> str | None:
    """Добавляет изображение номера в пак. Возвращает текст ошибки или None."""
    pack_type = pack["type"]
    # Для стикеров и эмодзи используем прозрачный фон без ватермарок.
    # Стикеры: 512px, эмодзи: 100px (требование Telegram).
    target_size = 100 if pack_type == "emoji" else 512
    processed = _plate_bytes_to_transparent(img_bytes, target_size)
...
def _plate_bytes_to_transparent(img_bytes: bytes, target_size: int) -> bytes:
    """
    Вырезает сам знак из полного изображения (с серым фоном) методом автокропа,
    убирает белый фон знака оставляя только рамку и текст,
    и возвращает PNG target_size*target_size с прозрачным фоном.
    """
    src = Image.open(io.BytesIO(img_bytes)).convert("RGBA")

    # 1. Найти bounding box знака: ищем не-серые пиксели (знак светлее/темнее фона #efefef)
    rgb = src.convert("RGB")
    rgb_data = rgb.load()
    W, H = rgb.size
    BG = (239, 239, 239)
    ...
    for y in range(H):
        for x in range(W):
            r, g, b = rgb_data[x, y]
            if not (abs(r - BG[0]) < TOLERANCE and
    ...
    # Если ничего не нашли — fallback на весь холст
    if max_x <= min_x or max_y <= min_y:
        min_x, min_y, max_x, max_y = 0, 0, W - 1, H - 1

    # 2. Кропаем знак с небольшим отступом
    PAD = 4
    crop_box = (max(0, min_x - PAD), max(0, min_y - PAD),
                min(W, max_x + PAD + 1), min(H, max_y + PAD + 1))
    cropped = src.crop(crop_box)

    # 3. Делаем белый/светлый фон прозрачным (фон самого знака = белый)
    pixels = cropped.load()
    cw, ch = cropped.size
    WHITE_THRESH = 230
    for cy_px in range(ch):
        for cx_px in range(cw):
            r, g, b, a = pixels[cx_px, cy_px]
            if r > WHITE_THRESH and g > WHITE_THRESH and b > WHITE_THRESH:
                pixels[cx_px, cy_px] = (r, g, b, 0)

    # 4. Вписываем в квадрат target_size с сохранением пропорций
    out = Image.new("RGBA", (target_size, target_size), (0, 0, 0, 0))
    ...
    # 5. Сохраняем с минимальным сжатием для максимального качества
    buf = io.BytesIO()
    out.save(buf, "PNG", optimize=False, compress_level=1)
    data_out = buf.getvalue()

    # 6. Если превышаем 500KB — пробуем сжать, но не уменьшаем изображение
...
async def _plate_save_start(update: Update, context: ContextTypes.DEFAULT_TYPE,
    ...
    if pack:
        ...
        err = await _plate_add_to_pack(context.bot, user_id, img_bytes, pack, emoji)
        try:
            await status.delete()
        except Exception:
            pass
        if err:
            ...
            url = pack_url(pack["name"])
            icon = "🖼" if pack_type == "sticker" else "✨"
            kb = [[InlineKeyboardButton(f"{icon} Открыть пак", url=url)]]
...
def main() -> None:
    ...
    # Путь к файлу с данными. На Railway подключи Volume и задай
    # переменную окружения DATA_DIR=/data  (или любой другой путь к Volume).
    # Без Volume данные сохраняются локально и живут пока жив контейнер.

...
def generate_plate_image(country: str, chars: str, region: str, region_name: str) -> bytes:
    ...
    if country == "RU":
        ...
        # KZ: ПОЛНОСТЬЮ БЕЛЫЙ номер. Слева флаг KZ (голубой прямоугольник как реальный флаг)
        # без цветной полосы-фона. Справа регион БЕЗ разделителя. Под флагом KZ.
        ...
        draw.rounded_rectangle([px, py, px+pw, py+ph], radius=8, fill="white",
                                outline="#111111", width=4)
        # Флаг KZ — голубой прямоугольник (2:1 соотношение) в левой зоне
        # Точно как реальный флаг: голубой фон, жёлтое солнце вверху-по центру,
        # под солнцем — золотой орёл, слева — вертикальный орнамент
        zone_w = 78
        fl_w   = 60   # ширина флага
        fl_h   = 40   # высота флага
        fl_x   = px + (zone_w - fl_w) // 2
        fl_y   = py + (ph - fl_h) // 2 - 8
        # Голубой фон флага
        draw.rectangle([fl_x, fl_y, fl_x+fl_w, fl_y+fl_h], fill="#00AFCA")
        # Вертикальный орнамент слева — чередующиеся жёлтые ромбы
        orn = 5
        for yi in range(fl_y+2, fl_y+fl_h-2, orn*2):
            draw.polygon([(fl_x+orn, yi), (fl_x+orn*2, yi+orn),
                          (fl_x+orn, yi+orn*2), (fl_x, yi+orn)], fill="#FFD700")
        # Солнце: 32 луча + круг
        s_cx = fl_x + int(fl_w * 0.6)
        s_cy = fl_y + int(fl_h * 0.38)
        for ai in range(0, 360, 11):
            a = _math.radians(ai)
            draw.line([(int(s_cx+6*_math.cos(a)), int(s_cy+6*_math.sin(a))),
                       (int(s_cx+10*_math.cos(a)), int(s_cy+10*_math.sin(a)))],
                      fill="#FFD700", width=1)
        draw.ellipse([s_cx-5, s_cy-5, s_cx+5, s_cy+5], fill="#FFD700")
        # Орёл под солнцем — крылья + тело
        e_cx = fl_x + int(fl_w * 0.6)
        e_cy = fl_y + int(fl_h * 0.72)
        draw.arc([e_cx-10, e_cy-5, e_cx+10, e_cy+5], start=195, end=345, fill="#FFD700", width=2)
        draw.line([(e_cx-10, e_cy), (e_cx-16, e_cy-10)], fill="#FFD700", width=2)
        draw.line([(e_cx+10, e_cy), (e_cx+16, e_cy-10)], fill="#FFD700", width=2)
        # "KZ" под флагом — чёрный, крупно
        fnt_kz = ImageFont.truetype(FONT_BOLD, 18)
        draw.text((fl_x + fl_w//2, fl_y + fl_h + 12), "KZ", fill="#111111", font=fnt_kz, anchor="mm")
        # Регион справа — БЕЗ разделителя
        right_w = 52
        rdx = px + pw - right_w - 6
        fnt_reg = ImageFont.truetype(FONT_BOLD, 36)
        draw.text((rdx + right_w//2, cy), region, fill="#111111", font=fnt_reg, anchor="mm")
        # Основной номер по центру
        c = chars.replace(" ", "")
        body = f"{c[:3]} {c[3:]}" if len(c) == 6 else chars
        fnt_pl = ImageFont.truetype(FONT_BOLD, 64)
        num_cx = px + zone_w + (rdx - px - zone_w) // 2
...
async def _push_sticker_to_tg(bot, user_id: int, pack_name: str, pack_type: str,
    ...
    if pack_type == "sticker":
        ...
        sticker_type = StickerType.CUSTOM_EMOJI  # needs_repainting handled by Telegram
    else:

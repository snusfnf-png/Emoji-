...
async def _prompt_first_sticker(update: Update, suffix: str, context=None) -> int:
    ...
    if is_adaptive:
        ...
        formats_text = "<i>Поддерживаются: фото/видео/стикер/гифка\n💎 Можно отправить премиум-эмодзи прямо в чат</i>"
    text = (
...
async def mgmt_add(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await query.edit_message_text(
        ...
        f"<i>💎 Поддерживаются премиум-эмодзи — отправь их прямо в чат</i>",
        parse_mode="HTML",

...
def process_image(raw_bytes: bytes, size: int, max_bytes: int = MAX_STICKER_BYTES) -> bytes:
    ...
    # Точное масштабирование с сохранением пропорций
...
async def get_sticker_data(bot, msg, pack_type: str = "sticker"):
    ...
    if msg.sticker:
        s = msg.sticker
        # is_animated/is_video надёжнее чем s.format в разных версиях PTB
...
async def _push_sticker_to_tg(bot, user_id: int, pack_name: str, pack_type: str,
    ...
    if isinstance(data, bytes):
        ...
        # Получаем строковое значение для API
        if hasattr(safe_fmt, "value"):
            fmt_for_upload = safe_fmt.value.strip()
        else:
            fmt_for_upload = str(safe_fmt).strip()
        logger.info("upload_sticker_file: fmt=%r", fmt_for_upload)
        uploaded = await bot.upload_sticker_file(
            user_id=user_id,
            sticker=upload_file,
            sticker_format=fmt_for_upload,
        )
    ...
    def make_sticker():
        # Определяем формат как строку — именно то что ждёт Telegram API
        safe_fmt = fmt if fmt is not None else StickerFormat.STATIC
        # Получаем строковое значение формата
        if hasattr(safe_fmt, "value"):
            fmt_str = safe_fmt.value   # "static", "animated", "video"
        else:
            fmt_str = str(safe_fmt)
        # Убираем пробелы/переносы на случай если PTB возвращает их
        fmt_str = fmt_str.strip()
        logger.info("make_sticker: fmt_str=%r", fmt_str)
        try:
    ...
    async def _create_set(sticker):
        ...
        try:
            # sticker_format как строка
            if hasattr(fmt, "value"):
                fmt_str_create = fmt.value.strip()
            else:
                fmt_str_create = str(fmt).strip() if fmt else "static"
            kwargs = dict(
                user_id=user_id,
                name=pack_name,
                title=title,

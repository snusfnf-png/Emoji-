...
async def copy_pack_receive_link(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    kb = [
        [InlineKeyboardButton("🖼 Стикер-пак",           callback_data="copy_as_sticker")],
        [InlineKeyboardButton("✨ Эмодзи-пак",            callback_data="copy_as_emoji")],
...
async def _do_copy_pack(update_or_msg, context: ContextTypes.DEFAULT_TYPE,
    ...
    """Выполняет копирование стикеров из исходного пака в новый или существующий.
    ...
    status_msg = await send_to.reply_text(
        "⏳ Копирую пак... Это может занять некоторое время."
    )
    ...
    for i, src_sticker in enumerate(stickers):
        try:
            # Обновляем статус каждые 5 стикеров
            if i % 5 == 0:
                await status_msg.edit_text(
                    f"⏳ Копирую стикеры... {i}/{total}"
                )

            # Получаем данные стикера
            src_fmt = src_sticker.format if hasattr(src_sticker, "format") and src_sticker.format else None
            if src_fmt is None:
            ...
            try:
                ...
                logger.warning("copy_pack: download failed for sticker %s: %s", src_sticker.file_id, dl_err)
                errors += 1
                continue

            # Конвертируем под целевой тип
            if target_type == "sticker":
                # Цель — обычный стикер-пак
            ...
            stickers_buf_dummy = [{}] * added  # имитируем буфер для определения is_new
            ...
            if err:
                logger.warning("copy_pack: push failed: %s", err)
                errors += 1
            else:
                added += 1

        except Exception as e:
            logger.warning("copy_pack: sticker %d error: %s", i, e)
            errors += 1
            continue
    ...
    result_text = (
        f"✅ <b>Готово!</b>\n\n"
        f"Скопировано: <b>{added}/{total}</b> стикеров\n"
    )
    if errors:
    ...
    # Сохраняем новый пак если это новый
    if creating_new:

...
async def receive_emoji_for_new(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        if data is None or fmt is None:
            ...
            return ADDING_STICKER
        logger.info("receive_emoji_for_new: fmt=%s data_len=%s", fmt, len(data) if isinstance(data, bytes) else "str")
...
async def _push_sticker_to_tg(bot, user_id: int, pack_name: str, pack_type: str,
    ...
    # PTB 20.7: InputSticker — без format; create_new_sticker_set — sticker_format обязателен
    def make_sticker():
    ...
    async def _create_set(sticker):
        ...
        # sticker_format обязателен в PTB 20.7 для create_new_sticker_set
        safe_fmt = fmt if fmt is not None else StickerFormat.STATIC
        kwargs = dict(
            user_id=user_id,
            name=pack_name,
    ...
    # add_sticker_to_set — format НЕ нужен, определяется по uploaded file

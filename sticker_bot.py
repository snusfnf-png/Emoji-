...
def process_image(raw_bytes: bytes, size: int, max_bytes: int = MAX_STICKER_BYTES) -> bytes:
    ...
    # Масштабируем с сохранением пропорций — вписываем в size x size
    ...
    # Сначала пробуем без уменьшения — только сжатие
    for compress in range(1, 10):
        ...
        if len(data) <= max_bytes:
            return data
    # Если всё ещё большой — уменьшаем картинку (только если реально нужно)
    scale = 0.9
    while scale > 0.3:
        sw = max(1, int(new_w * scale))
        sh = max(1, int(new_h * scale))
        resized = img.resize((sw, sh), Image.LANCZOS)
        final = Image.new("RGBA", (size, size), (0, 0, 0, 0))
        off = ((size - sw) // 2, (size - sh) // 2)
...
def process_image_for_sticker(raw_bytes: bytes, pack_type: str = "sticker") -> bytes:
    # стикер: 512x512, эмодзи: 100x100
    size = 512 if pack_type == "sticker" else 100
    # лимит: стикер 500кб, эмодзи 100кб
    max_b = MAX_STICKER_BYTES if pack_type == "sticker" else 100 * 1024
    return process_image(raw_bytes, size, max_b)
...
def convert_to_webm(raw_bytes: bytes, pack_type: str = "sticker") -> bytes | None:
    """Конвертирует MP4/GIF в WEBM (VP9) для видео-стикера Telegram.
    sticker: 512x512, макс 256кб, макс 3 сек
    emoji:   100x100, макс 100кб, макс 3 сек
    """
    import subprocess, tempfile, os
    ffmpeg = _get_ffmpeg_path()
    is_emoji = "emoji" in pack_type
    dim      = 100 if is_emoji else 512
    max_kb   = 90  if is_emoji else 240   # немного ниже лимита TG
    with tempfile.TemporaryDirectory() as tmpdir:
        in_path  = os.path.join(tmpdir, "input.mp4")
        out_path = os.path.join(tmpdir, "output.webm")
        with open(in_path, "wb") as f:
            f.write(raw_bytes)
        # scale вписывает в dim×dim с прозрачным/чёрным паддингом
        pad_color = "black@0" if is_emoji else "black@0"
        vf = (
            f"scale={dim}:{dim}:force_original_aspect_ratio=decrease,"
            f"pad={dim}:{dim}:(ow-iw)/2:(oh-ih)/2:color={pad_color},"
            f"fps={'24' if not is_emoji else '30'}"
        )
        # Для эмодзи нужен прозрачный фон — используем yuva420p / rgba
        pix_fmt_args = ["-pix_fmt", "yuva420p"] if is_emoji else ["-pix_fmt", "yuv420p"]
        bitrate = "60k" if is_emoji else "200k"
        crf     = "33"  if is_emoji else "30"
        cmd = [
            ffmpeg, "-y", "-i", in_path,
            "-t", "3",
            "-vf", vf,
        ...
        try:
            ...
            logger.info("convert_to_webm: size=%d bytes (limit %dkb)", len(data), max_kb)
            # Если всё равно большой — повторяем с меньшим битрейтом
            if len(data) > max_kb * 1024:
                ...
                result2 = subprocess.run(cmd2, capture_output=True, timeout=90)
                if result2.returncode == 0:
                    data = open(out_path, "rb").read()
                    logger.info("convert_to_webm retry: size=%d bytes", len(data))
            return data
        except FileNotFoundError:
            logger.warning("ffmpeg не найден — видео-стикеры недоступны")
            return None
        except Exception as e:
...
async def get_sticker_data(bot, msg, pack_type: str = "sticker"):
    ...
    if msg.sticker:
        ...
        if fmt == StickerFormat.STATIC:
            # статичный стикер → обрабатываем как изображение
            return safe_process_image(raw, pack_type), StickerFormat.STATIC
        if fmt == StickerFormat.VIDEO:
            # видео-стикер (webm) — переконвертируем под нужный размер
            webm = convert_to_webm(raw, pack_type)
            if webm:
                return webm, StickerFormat.VIDEO
            return raw, StickerFormat.VIDEO
        if fmt == StickerFormat.ANIMATED:
            # TGS анимация — возвращаем как есть (TG сам поймёт)
    ...
    if msg.document:
        ...
        # GIF → конвертируем в WEBM
        if "gif" in mime or name.endswith(".gif"):
            raw = await dl(d.file_id)
            webm = convert_to_webm(raw, pack_type)
            if webm:
                return webm, StickerFormat.VIDEO
            # ffmpeg недоступен — берём первый кадр как статичный
            try:
                ...
                return None, None
        # MP4 / MOV / видео-документ → конвертируем в WEBM
    ...
    try:
        ...
        if data is None:
            await msg.reply_text(
                ...
                "Для видео/гифок убедись что на сервере установлен <b>ffmpeg</b>.\n"
                "Для фото — отправь PNG, WEBP или JPG.",
                parse_mode="HTML",
            )
            return ADDING_STICKER

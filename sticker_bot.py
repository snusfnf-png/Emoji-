...
def recolor_webp(raw: bytes, hex_color: str) -> bytes:
    """
    Перекраска статичного WebP-стикера алгоритмом JellyColor:
    воспринимаемая яркость пикселя умножается на целевой цвет.
    Альфа-канал сохраняется без изменений.
    """
    h = hex_color.lstrip("#")
    r_target = int(h[0:2], 16)
    g_target = int(h[2:4], 16)
    b_target = int(h[4:6], 16)
    ...
    for y in range(img.height):
        for x in range(img.width):
            ...
            if ao > 0:
                ...
                data[x, y] = (
                    int(r_target * gray / 255),
                    int(g_target * gray / 255),
                    int(b_target * gray / 255),
                    ao,
                )
...
def _tint_lottie(lottie_json: dict, hex_color: str) -> dict:
    """
    Перекраска Lottie JSON алгоритмом JellyColor:
    обходит все цветовые поля ("c") и применяет
    воспринимаемую яркость * целевой цвет.
    Поддерживает статичные цвета и ключевые кадры.
    """
    h = hex_color.lstrip("#")
    nr = int(h[0:2], 16) / 255.0
    ng = int(h[2:4], 16) / 255.0
    nb = int(h[4:6], 16) / 255.0
        ...
        if isinstance(obj, dict):
            if "c" in obj and isinstance(obj["c"], dict) and "k" in obj["c"]:
                ...
                if isinstance(k, list) and len(k) >= 3 and isinstance(k[0], (int, float)):
                    # Статичный цвет [r, g, b] или [r, g, b, a]
                    gray = 0.299 * k[0] + 0.587 * k[1] + 0.114 * k[2]
                    obj["c"]["k"] = [nr * gray, ng * gray, nb * gray] + (k[3:] if len(k) > 3 else [1.0])
                    ...
                    for kf in k:
                        if isinstance(kf, dict) and "s" in kf:
                            ...
                            if isinstance(s, list) and len(s) >= 3:
                                ...
                                kf["s"] = [nr * gray, ng * gray, nb * gray] + (s[3:] if len(s) > 3 else [1.0])
            for v in obj.values():
                _walk(v)
...
def recolor_tgs(raw: bytes, hex_color: str) -> bytes:
    """Перекрашивает TGS-стикер (gzip Lottie JSON) алгоритмом JellyColor."""
    lottie = json.loads(gzip.decompress(raw))
    _tint_lottie(lottie, hex_color)
...
def recolor_webm(raw: bytes, hex_color: str) -> bytes | None:
    """Перекрашивает WEBM видео-стикер через ffmpeg color overlay."""
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        with open(in_path, "wb") as f:
            f.write(raw)
        # grayscale → умножить на целевой цвет (корректно для белого и всех цветов)
...
def _paint_kb_colors(last: str | None = None) -> InlineKeyboardMarkup:
    ...
    if row:
        rows.append(row)
    rows.append([InlineKeyboardButton("Свой HEX цвет", callback_data="paint_clr:custom", icon_custom_emoji_id="6050679691004612757")])
    rows.append([InlineKeyboardButton("Назад", callback_data="begin", icon_custom_emoji_id="5893057118545646106")])
...
def _paint_kb_scope() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        [
            InlineKeyboardButton("Один стикер", callback_data="paint_scope:single", icon_custom_emoji_id="6035128606563241721"),
            InlineKeyboardButton("Весь пак", callback_data="paint_scope:pack", icon_custom_emoji_id="5884479287171485878"),
        ],
        [InlineKeyboardButton("Назад", callback_data="begin", icon_custom_emoji_id="5893057118545646106")],
    ])
...
async def paint_start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await query.edit_message_text(
        ...
        parse_mode="HTML",
        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Назад", callback_data="begin", icon_custom_emoji_id="5893057118545646106")]]),
    )
    return PAINT_SCOPE
...
async def paint_receive_sticker(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if is_custom_emoji:
        ...
        await update.message.reply_text(
            ...
            parse_mode="HTML",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("В меню", callback_data="begin", icon_custom_emoji_id="5873147866364514353")]]),
        )
        return PAINT_SCOPE
...
async def _paint_execute(update: Update, context: ContextTypes.DEFAULT_TYPE, hex_color: str):
    ...
    status = await msg.reply_text("<tg-emoji emoji-id='5345906554510012647'>🔄</tg-emoji> <b>Перекрашиваю...</b>", parse_mode="HTML")
    ...
    try:
        if mode == "single":
            ...
            if pack_created:
                ...
                item_label = "эмодзи" if is_emoji_pack else "стикеров"
                # Сохраняем данные для кнопки "Сохранить"
                context.user_data["paint_result_pack_name"]  = new_pack_name
                context.user_data["paint_result_pack_title"] = new_pack_title
                context.user_data["paint_result_pack_type"]  = "emoji" if is_emoji_pack else "sticker"
                context.user_data["paint_result_pack_count"] = ok
                result_text = (
                    f"✅ <b>Готово!</b>\n\n"
                    f"Перекрашено: <b>{ok}/{total}</b> {item_label}\n"
                    f"🖌 Цвет: <code>{hex_color}</code>\n\n"
                    f"<b>{new_pack_title}</b>"
                )
                kb_paint = [
                    [InlineKeyboardButton("Открыть пак", url=pack_link, icon_custom_emoji_id="5769289093221454192")],
                    [InlineKeyboardButton("Сохранить", callback_data="paint_save_pack", icon_custom_emoji_id="6039802767931871481")],
                    [InlineKeyboardButton("В меню", callback_data="begin", icon_custom_emoji_id="5873147866364514353")],
                ]
                await msg.reply_text(
                    result_text,
                    parse_mode="HTML",
                    disable_web_page_preview=True,
                    reply_markup=InlineKeyboardMarkup(kb_paint),
                )
            else:
                await msg.reply_text(
...
async def paint_save_pack(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Сохранить покрашенный пак в список паков пользователя."""
    query = update.callback_query
    await query.answer()
    user_id    = update.effective_user.id
    pack_name  = context.user_data.get("paint_result_pack_name")
    pack_title = context.user_data.get("paint_result_pack_title", "Покрашенный пак")
    pack_type  = context.user_data.get("paint_result_pack_type", "sticker")
    pack_count = context.user_data.get("paint_result_pack_count", 0)
    ...
    # Переходим сразу к управлению паком
    idx = len(packs) - 1
    ...
    kb = [
        [InlineKeyboardButton("Открыть пак", url=url, icon_custom_emoji_id="5769289093221454192")],
        [InlineKeyboardButton("Переименовать пак", callback_data="mgmt_rename", icon_custom_emoji_id="5870676941614354370")],
        [InlineKeyboardButton(add_label,                 callback_data="mgmt_add")],
        [InlineKeyboardButton(delete_label, callback_data="mgmt_delete", icon_custom_emoji_id="5870875489362513438")],
        [InlineKeyboardButton("Сменить иконку пака", callback_data="mgmt_icon", icon_custom_emoji_id="6035128606563241721")],
        [InlineKeyboardButton(paint_label, callback_data=f"paint_from_pack:{pack['name']}")],
        [InlineKeyboardButton("Удалить весь пак", callback_data="mgmt_delete_all", icon_custom_emoji_id="5870657884844462243")],
        [InlineKeyboardButton("К списку паков", callback_data="list_packs", icon_custom_emoji_id="5893057118545646106")],
    ]
    await query.edit_message_reply_markup(reply_markup=None)
    await query.message.reply_text(
        f"{icon} <b>{pack['title']}</b>\n{item_word.capitalize()}: {n}\n\n"
        f"✅ <i>Пак сохранён — можешь управлять им прямо здесь</i>",
        parse_mode="HTML",
...
def back_btn(label: str = "Назад", data: str = "begin") -> list:
    return [InlineKeyboardButton(label, callback_data=data, icon_custom_emoji_id="5893057118545646106")]
...
async def send_main_menu(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    kb = [
        [InlineKeyboardButton("Создать стикер-пак", callback_data="create_sticker", icon_custom_emoji_id="5870528606328852614")],
        [InlineKeyboardButton("Создать эмодзи-пак", callback_data="create_emoji", icon_custom_emoji_id="5870764288364252592")],
        [InlineKeyboardButton("Создать номерной знак", callback_data="create_plate", icon_custom_emoji_id="5873147866364514353")],
        [InlineKeyboardButton("Скопировать пак", callback_data="copy_pack", icon_custom_emoji_id="5870528606328852614")],
        [InlineKeyboardButton("Покрасить стикер", callback_data="paint_start", icon_custom_emoji_id="6050679691004612757")],
    ]
    text = (
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    text = (
        f'<tg-emoji emoji-id="5870994129244131212">👤</tg-emoji> <b>Профиль {name}</b>\n\n'
        f"<blockquote>"
        f'<tg-emoji emoji-id="5870528606328852614">📁</tg-emoji> Стикеров: <b>{sticker_count}</b>\n'
        f'<tg-emoji emoji-id="5870764288364252592">🙂</tg-emoji> Эмодзи: <b>{emoji_count}</b>\n'
        f'<tg-emoji emoji-id="5873147866364514353">🏘</tg-emoji> Номеров: <b>{plate_count}</b>\n'
        f'<tg-emoji emoji-id="5884479287171485878">📦</tg-emoji> Покупок: <b>{purchases}</b>\n'
        f'<tg-emoji emoji-id="5904462880941545555">🪙</tg-emoji> Потрачено: <b>{stars_spent}</b>\n'
        f'<tg-emoji emoji-id="5890937706803894250">📅</tg-emoji> Регистрация: <b>{reg_str}</b>'
        f"</blockquote>"
    )

    kb = [
        [InlineKeyboardButton("Меню", callback_data="start_begin", icon_custom_emoji_id="5873147866364514353"),
         InlineKeyboardButton("Мои паки", callback_data="start_packs", icon_custom_emoji_id="5870528606328852614")],
        [InlineKeyboardButton("Поддержка", url="https://t.me/tntks", icon_custom_emoji_id="6028435952299413210")],
    ]
...
async def start_packs_cb(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if not packs:
        await query.message.reply_text(
            ...
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("В меню", callback_data="start_begin", icon_custom_emoji_id="5873147866364514353")]]),
        )
        return MAIN_MENU
...
def _admin_main_kb():
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("Пользователи", callback_data="adm_users", icon_custom_emoji_id="5870772616305839506")],
        [InlineKeyboardButton("Статистика", callback_data="adm_stats", icon_custom_emoji_id="5870921681735781843")],
        [InlineKeyboardButton("Рассылка", callback_data="adm_broadcast", icon_custom_emoji_id="6039422865189638057")],
        [InlineKeyboardButton("Отложенная рассылка", callback_data="adm_schedule", icon_custom_emoji_id="5983150113483134607")],
        [InlineKeyboardButton("Цены", callback_data="adm_prices", icon_custom_emoji_id="5904462880941545555")],
        [InlineKeyboardButton("Тех. перерыв", callback_data="adm_maintenance", icon_custom_emoji_id="5870982283724328568")],
    ])
...
async def adm_users(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await q.edit_message_text(
        ...
        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Назад", callback_data="adm_back", icon_custom_emoji_id="5893057118545646106")]]),
    )
    return ADMIN_MENU
...
def _stats_text_and_kb(stats: dict, day_users: int, day=None, page: int = 0, total_pages: int = 1) -> tuple:
    ...
    text = (
        f"{header}\n"
        "━━━━━━━━━━━━━━━━\n"
        f'<tg-emoji emoji-id="5870994129244131212">👤</tg-emoji> Пользователей:     <b>{day_users}</b>\n'
        f'<tg-emoji emoji-id="5870528606328852614">📁</tg-emoji> Стикер-паков:      <b>{sp}</b>\n'
        f'<tg-emoji emoji-id="5870764288364252592">🙂</tg-emoji> Эмодзи-паков:      <b>{ep}</b>\n'
        f'<tg-emoji emoji-id="5873147866364514353">🏘</tg-emoji> Номерных знаков:   <b>{pl}</b>\n'
        f'<tg-emoji emoji-id="5870982283724328568">⚙</tg-emoji> Стикеров в паках:  <b>{sa}</b>\n'
        f'<tg-emoji emoji-id="5870982283724328568">⚙</tg-emoji> Эмодзи в паках:    <b>{ea}</b>\n'
        "━━━━━━━━━━━━━━━━"
    )
    if total_pages > 1:
    ...
    nav = []
    if page > 0:
        nav.append(InlineKeyboardButton("Новее", callback_data=f"adm_stats_p_{page - 1}", icon_custom_emoji_id="5893057118545646106"))
    if page < total_pages - 1:
        nav.append(InlineKeyboardButton("Старее", callback_data=f"adm_stats_p_{page + 1}", icon_custom_emoji_id="5893057118545646106"))

    kb = []
    if nav:
        kb.append(nav)
    if day is not None:
        kb.append([InlineKeyboardButton("За все время", callback_data="adm_stats_p_0", icon_custom_emoji_id="5870921681735781843")])
    kb.append([InlineKeyboardButton("В меню", callback_data="adm_back", icon_custom_emoji_id="5893057118545646106")])
...
async def adm_broadcast_start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await q.edit_message_text(
        ...
        parse_mode="HTML",
        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Отмена", callback_data="adm_back", icon_custom_emoji_id="5870657884844462243")]]),
    )
...
async def adm_schedule_start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await q.edit_message_text(
        "<tg-emoji emoji-id='5983150113483134607'>⏰</tg-emoji> <b>Отложенная рассылка</b>\n\n"
...
async def adm_prices(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    q = update.callback_query
    await q.answer()
    lines = ["<tg-emoji emoji-id='5904462880941545555'>🪙</tg-emoji> <b>Текущие цены:</b>\n"]
    kb = []
    for key, label in PRICE_LABELS.items():
        val = DB.get_setting(key, "1")
        lines.append(f"• {label}: <b>{val} ⭐</b>")
        kb.append([InlineKeyboardButton(label, callback_data=f"adm_setprice_{key}", icon_custom_emoji_id="5870676941614354370")])
    kb.append([InlineKeyboardButton("Назад", callback_data="adm_back", icon_custom_emoji_id="5893057118545646106")])
    await q.edit_message_text(
...
async def select_pack(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    kb = [
        [InlineKeyboardButton("Открыть пак", url=url, icon_custom_emoji_id="5769289093221454192")],
        [InlineKeyboardButton("Переименовать пак", callback_data="mgmt_rename", icon_custom_emoji_id="5870676941614354370")],
        *([[InlineKeyboardButton(add_label, callback_data="mgmt_add", icon_custom_emoji_id="5870633910337015697")]] if not is_plate else []),
        [InlineKeyboardButton(delete_label, callback_data="mgmt_delete", icon_custom_emoji_id="5870875489362513438")],
        *([[InlineKeyboardButton("Сменить иконку пака", callback_data="mgmt_icon", icon_custom_emoji_id="6035128606563241721")]] if not is_plate else []),
        *([[InlineKeyboardButton(paint_label, callback_data=f"paint_from_pack:{pack['name']}", icon_custom_emoji_id="6035128606563241721")]] if not is_plate else []),
        [InlineKeyboardButton("Удалить весь пак", callback_data="mgmt_delete_all", icon_custom_emoji_id="5870657884844462243")],
        back_btn("К списку паков", "list_packs"),
    ]
    await query.edit_message_text(
        f"{icon} <b>{pack['title']}</b>\n{item_word.capitalize()}: {n}",
        parse_mode="HTML",
...
async def start_create(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if pack_type == "emoji":
        kb = [[InlineKeyboardButton("Сделать адаптивным", callback_data="set_adaptive", icon_custom_emoji_id="5870633910337015697")]]
        await query.edit_message_text(
...
async def receive_pack_name(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    kb = [[InlineKeyboardButton("Случайная ссылка", callback_data="random_link", icon_custom_emoji_id="6041731551845159060")]]
    await update.message.reply_text(
...
async def _finish_batch(update_or_msg, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    kb = [
        [InlineKeyboardButton("Открыть пак", url=url, icon_custom_emoji_id="5769289093221454192")],
        [InlineKeyboardButton("Сохранить пак", callback_data="save_pack", icon_custom_emoji_id="5884479287171485878")],
    ]
...
async def mgmt_add(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await query.edit_message_text(
        ...
        parse_mode="HTML",
        reply_markup=InlineKeyboardMarkup([back_btn("Отмена", f"select_pack_{idx}")]),
    )
        ...
        if context.user_data.get("fire_emoji_confirmed_mgmt"):
            ...
            await msg.reply_text(
                ...
                reply_markup=InlineKeyboardMarkup([
                    [InlineKeyboardButton("К паку", callback_data=f"select_pack_{idx}", icon_custom_emoji_id="5870982283724328568")],
                ]),
            )
            return ADD_STICKER_FILE
...
async def fire_all_confirm_new(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await query.message.reply_text(
        ...
        parse_mode="HTML",
        reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton("Да, везде 🔥", callback_data="fire_all_yes_new", icon_custom_emoji_id="5870633910337015697"),
             InlineKeyboardButton("Нет", callback_data="fire_all_no_new", icon_custom_emoji_id="5870657884844462243")],
        ]),
    )
    return WAITING_EMOJI
...
async def fire_all_confirm_mgmt(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await query.message.reply_text(
        ...
        parse_mode="HTML",
        reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton("Да, везде 🔥", callback_data="fire_all_yes_mgmt", icon_custom_emoji_id="5870633910337015697"),
             InlineKeyboardButton("Нет", callback_data="fire_all_no_mgmt", icon_custom_emoji_id="5870657884844462243")],
        ]),
    )
    return ADD_STICKER_EMOJI
...
async def mgmt_delete(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await query.edit_message_text(
        ...
        reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton("Открыть пак", url=url, icon_custom_emoji_id="5769289093221454192")],
            back_btn("Отмена", f"select_pack_{idx}"),
        ]),
    )
...
async def confirm_delete_sticker(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        await msg.reply_text(
            ...
            reply_markup=InlineKeyboardMarkup([back_btn("Назад", f"select_pack_{idx}")]),
        )
        return DELETE_STICKER
    ...
    await msg.reply_text(
        ...
        reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton("Удалить ещё", callback_data="mgmt_delete", icon_custom_emoji_id="5870875489362513438")],
            [InlineKeyboardButton("К паку", callback_data=f"select_pack_{idx}", icon_custom_emoji_id="5870982283724328568")],
        ]),
    )
    return PACK_SELECTED
...
async def mgmt_delete_all(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await query.edit_message_text(
        ...
        parse_mode="HTML",
        reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton("Да, удалить", callback_data=f"confirm_delete_pack_{idx}", icon_custom_emoji_id="5870633910337015697")],
            back_btn("Отмена", f"select_pack_{idx}"),
        ]),
    )
    return PACK_SELECTED
...
def _region_page_kb(country: str, page: int) -> InlineKeyboardMarkup:
    ...
    if start + REGIONS_PER_PAGE < total:
        nav.append(InlineKeyboardButton("▶️", callback_data=f"prp_{country}_{page+1}"))
    kb.append(nav)
    kb.append(back_btn("Назад к странам", "create_plate"))
...
async def plate_select_region(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await query.edit_message_text(
        ...
        parse_mode="HTML",
        reply_markup=InlineKeyboardMarkup([
            back_btn("Сменить регион", f"pc_{country}")
        ]),
    )
    return PLATE_INPUT
...
async def plate_receive_input(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        kb = [
            [InlineKeyboardButton("В стикер-пак ⭐️", callback_data="plate_buy_sticker", icon_custom_emoji_id="6035128606563241721"),
             InlineKeyboardButton("В эмодзи-пак ⭐️", callback_data="plate_buy_emoji", icon_custom_emoji_id="5940660740758184142")],
            [InlineKeyboardButton("Другой номер", callback_data=f"plate_other_{country}", icon_custom_emoji_id="5345906554510012647")],
            [InlineKeyboardButton("В меню", callback_data="plate_menu", icon_custom_emoji_id="5873147866364514353")],
        ]
        try:
...
async def plate_pack_receive_name(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    kb = [[InlineKeyboardButton("Случайная ссылка", callback_data="plate_random_link", icon_custom_emoji_id="6041731551845159060")]]
    await update.message.reply_text(
...
async def copy_pack_start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await query.edit_message_text(
        "<tg-emoji emoji-id='5870528606328852614'>📁</tg-emoji> <b>Копирование пака</b>\n\n"
...
async def copy_pack_receive_link(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    kb = [
        ...
        [InlineKeyboardButton("Добавить в созданный", callback_data="copy_add_to_existing", icon_custom_emoji_id="5870657884844462243")],
        [InlineKeyboardButton("Назад", callback_data="begin", icon_custom_emoji_id="5893057118545646106")],
    ]
    await update.message.reply_text(
...
async def _do_copy_pack(update_or_msg, context: ContextTypes.DEFAULT_TYPE,
    ...
    kb = [[InlineKeyboardButton("Открыть пак", url=url, icon_custom_emoji_id="5769289093221454192")]]

    if creating_new:
        kb.append([InlineKeyboardButton("Сохранить", callback_data="copy_save_pack", icon_custom_emoji_id="6039802767931871481")])
    ...
    kb.append([InlineKeyboardButton("В меню", callback_data="begin", icon_custom_emoji_id="5873147866364514353")])
...
async def copy_add_to_existing(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if not packs:
        await query.edit_message_text(
            ...
            "/cancel — отмена",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Назад", callback_data="copy_pack", icon_custom_emoji_id="5893057118545646106")]]),
        )
        return COPY_PACK_TYPE
    ...
    kb.append([InlineKeyboardButton("Назад", callback_data="copy_pack", icon_custom_emoji_id="5893057118545646106")])
...
async def copy_save_pack(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await query.message.reply_text(
        ...
        reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton("В меню", callback_data="begin", icon_custom_emoji_id="5873147866364514353")],
            [InlineKeyboardButton("Мои паки", callback_data="list_packs", icon_custom_emoji_id="5870528606328852614")],
        ]),
    )
    return COPY_PACK_TYPE

    ...
    try:
        if mode == "single":
            ...
            if pack_created:
                ...
                # Сохраняем данные о покрашенном паке для возможного сохранения
...
async def paint_save_pack(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Callback: сохранить покрашенный пак в мои паки."""
    query = update.callback_query
    await query.answer()
    ...
    if not pack_info:
        await query.answer("⚠️ Нет данных о паке. Попробуй покрасить снова.", show_alert=True)
        return PAINT_SCOPE

    existing = get_packs(context, user_id)
    if any(p.get("name") == pack_info["name"] for p in existing):
        await query.answer("Этот пак уже есть в твоих паках!", show_alert=True)
        return PAINT_SCOPE

    existing.append({
        "title":   pack_info["title"],
        "name":    pack_info["name"],
        "suffix":  pack_info["name"].rsplit("_", 1)[-1],
        "type":    pack_info["type"],
    ...
    await query.edit_message_reply_markup(
        reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton("✅ Сохранено в мои паки", callback_data="noop_paint")],

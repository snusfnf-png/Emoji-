...
async def send_main_menu(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    text = (
        ...
        "• <b>Скопировать пак</b> — скопировать чужой набор"
    )
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    kb = [
        ...
        [InlineKeyboardButton("Покрасить стикер 🖼️", url="https://t.me/Meganaytbot?start=start"),
        ...
        if not stickers_buf:
            ...
            try:
                ...
                if ("already occupied" in err_str.lower()
                    ...
                    return None
                # RetryAfter внутри push — пробрасываем текст с временем
...
async def _do_copy_pack(update_or_msg, context: ContextTypes.DEFAULT_TYPE,
    ...
    for i, src_sticker in enumerate(stickers):
        # Обновляем статус каждые 3 стикера
        if i % 3 == 0:
            try:
        ...
        # ── Скачиваем файл с ретраями ─────────────────────────────────────────
        # КЛЮЧЕВАЯ ПРОБЛЕМА: custom_emoji стикеры часто нельзя скачать через
        # get_file — Telegram возвращает ошибку или пустой файл.
        # Стратегия:
        #   1) Для custom_emoji — сначала пробуем get_custom_emoji_stickers
        #      чтобы получить актуальный file_id, потом скачиваем его.
        #   2) Если скачать не удалось — используем file_id напрямую (без байт),
        #      Telegram сам может принять file_id существующего стикера.
        #   3) Обычные стикеры — скачиваем байты как обычно.

        raw      = None
        use_file_id_directly = False  # fallback: передать file_id без скачивания

        # Для custom_emoji пробуем получить свежий file_id через get_custom_emoji_stickers
            ...
            if custom_emoji_id:
                try:
                    ...
                    if fresh:
                        ...
                        logger.info("copy_pack: got fresh file_id for custom_emoji %s", custom_emoji_id)
                except Exception as ce_err:
                    logger.warning("copy_pack: get_custom_emoji_stickers failed: %s", ce_err)
        ...
        for dl_attempt in range(5):
            try:
                ...
                elif "flood" in err_s.lower() or "retry" in err_s.lower():
                    wait = 10 * (dl_attempt + 1)
                else:
                    wait = 3 * (dl_attempt + 1)
                logger.info("copy_pack: download error sticker %d (attempt %d), waiting %ds: %s",
                            i, dl_attempt, wait, dl_err)
                await asyncio.sleep(wait)
            except Exception as dl_err:
                logger.warning("copy_pack: download exception sticker %d (attempt %d): %s",
                               i, dl_attempt, dl_err)
                await asyncio.sleep(3)

        # Если скачать не удалось — для custom_emoji используем file_id напрямую.
        # Telegram Bot API принимает file_id существующего стикера в InputSticker.
        if not raw and is_src_custom_emoji:
            logger.info("copy_pack: using file_id directly for custom_emoji sticker %d", i)
            use_file_id_directly = True

        if not raw and not use_file_id_directly:
            logger.warning("copy_pack: skipping sticker %d — download failed after all attempts", i)
            errors += 1
            continue

        # ── Конвертация ───────────────────────────────────────────────────────
        if use_file_id_directly:
            # Передаём file_id напрямую — Telegram сам обработает
            data       = actual_file_id
            fmt        = src_fmt
            push_type  = target_type
            push_is_ce = True  # сообщаем _push_sticker_to_tg что это file_id
        else:
            try:
                ...
                # Fallback: используем сырые данные без конвертации
        ...
        stickers_buf_dummy = [{}] * added  # пустой = создать пак, не пустой = добавить

        # ── Пушим в Telegram с ретраями ───────────────────────────────────────
        # ПРИЧИНА ПРОПУСКА 3: ретрай прерывался на любой ошибке, не только flood.
        # Теперь ретраим ВСЕ временные ошибки, читаем точный retry_after.
        pack_full = False
        push_ok   = False
        for push_attempt in range(5):
            ...
            err_s = str(err)
            logger.warning("copy_pack: push attempt %d sticker %d error: %s",
                           push_attempt, i, err_s)

            if "STICKERS_TOO_MUCH" in err_s or "максимальное" in err_s:
                pack_full = True
                break

            # Читаем точное время ожидания из retry:N
            if err_s.startswith("retry:"):
                ...
                wait = 10 * (push_attempt + 1)
            elif "timeout" in err_s.lower() or "timed out" in err_s.lower():
                wait = 5
            elif "bad gateway" in err_s.lower() or "network" in err_s.lower():
                wait = 5 * (push_attempt + 1)
            else:
                wait = 3 * (push_attempt + 1)

            logger.info("copy_pack: waiting %ds before retry", wait)
            await asyncio.sleep(wait)
        ...
        if pack_full:
            try:
                ...
                pass
            break  # дальше копировать некуда

        if push_ok:
            added += 1
            # ПРИЧИНА ПРОПУСКА 4: слишком маленькая пауза → флуд → пропуски.
            # 0.5с достаточно и не даёт флуда при любом размере пака.
    ...
    if added == 0:
        await status_msg.edit_text(
            f"❌ Не удалось скопировать ни одного стикера.\nОшибок: {errors}"
        )
        return CHOOSE_TYPE

    url         = pack_url(target_pack_name)
    item_label  = "эмодзи" if target_type == "emoji" else "стикеров"
    result_text = f"✅ <b>Готово!</b>\n\nСкопировано: <b>{added}/{total}</b> {item_label}\n"
    if errors:
        result_text += f"Пропущено: {errors}\n"
    result_text += f"\n<b>{target_pack_title}</b>"
...
async def copy_into_pack_selected(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    target_type  = pack["type"] if pack["type"] in ("sticker", "emoji") else "sticker"

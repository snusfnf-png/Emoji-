...
class DB:
    ...
    def get_stats_days(cls) -> list:
        """Возвращает список дат (DATE) у которых есть дневная статистика, сортировка DESC."""
        if not DATABASE_URL:
            return []
        try:
            conn = cls._get_conn()
            with conn.cursor() as cur:
                cur.execute("""
                    SELECT DISTINCT day FROM daily_stats
                    ORDER BY day DESC
                """)
    ...
    def get_stats_for_day(cls, day) -> dict:
        """Возвращает словарь stat_key→value за конкретный день."""
        if not DATABASE_URL:
            return {}
        try:
            conn = cls._get_conn()
            with conn.cursor() as cur:
                cur.execute(
                    "SELECT stat_key, stat_value FROM daily_stats WHERE day = %s",
                    (day,)
                )
                return {row[0]: row[1] for row in cur.fetchall()}
        except Exception as e:
    ...
    def get_daily_users_count(cls, day) -> int:
        ...
        try:
            ...
            with conn.cursor() as cur:
                cur.execute(
                    "SELECT COUNT(*) FROM daily_users WHERE day = %s", (day,)
                )
                row = cur.fetchone()
...
# ── Статистика ────────────────────────────────────────────────────────────────
def _build_stats_text(stats: dict, day_users: int, day=None, page: int = 0, total_pages: int = 1) -> tuple:
    """Строит текст и клавиатуру для страницы статистики."""
    ...
    if day is None:
        title = "📊 <b>Статистика за все время</b>"
        date_line = ""
    else:
        try:
            ...
            date_line = f"\n🗓 <b>{day}</b>"
        title = f"📊 <b>Статистика за день</b>{date_line}"
    ...
    nav = []
    if page > 0:
        nav.append(InlineKeyboardButton("◀️ Новее", callback_data=f"adm_stats_page_{page - 1}"))
    if page < total_pages - 1:
        nav.append(InlineKeyboardButton("Старее ▶️", callback_data=f"adm_stats_page_{page + 1}"))

    # Кнопка "Все время" только если смотрим день
    extra = []
    if day is not None:
        extra.append(InlineKeyboardButton("📊 Всё время", callback_data="adm_stats_all"))
...
async def adm_stats(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Показывает статистику за все время (стартовая страница)."""
    q = update.callback_query
    await q.answer()
    stats      = DB.get_stats()
    total_users = len(DB.get_all_users())
    days       = DB.get_stats_days()
    total_pages = len(days) + 1  # +1 страница «все время»

    text, kb = _build_stats_text(
        stats=stats,
        day_users=total_users,
        day=None,
        page=0,
...
async def adm_stats_page(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Переключение по страницам статистики. page=0 → все время, page=1..N → день."""
    q = update.callback_query
    await q.answer()
    page = int(q.data.replace("adm_stats_page_", ""))
    days = DB.get_stats_days()
    total_pages = len(days) + 1
...
async def adm_stats_all(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Кнопка 'Всё время' — возврат на страницу 0."""
    update.callback_query.data = "adm_stats_page_0"
    return await adm_stats_page(update, context)


# ── Новые по дням (редирект на adm_stats) ──────────────────────────────────────
    ...
    conv = ConversationHandler(
        ...
        states={
            ...
            ADMIN_MENU: [
                ...
                CallbackQueryHandler(adm_stats_all,        pattern="^adm_stats_all$"),
                CallbackQueryHandler(adm_stats_page,       pattern="^adm_stats_page_\\d+$"),

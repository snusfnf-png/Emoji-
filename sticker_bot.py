...
async def paint_start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await query.edit_message_text(
        "🎨 <b>Покраска стикера</b>\n\n"
        "Отправь мне стикер или ссылку на стикер-пак:\n"
        "<code>t.me/addstickers/ИМЯ_ПАКА</code>\n\n"
        "<i>Поддерживаются статичные WebP и анимированные TGS стикеры.</i>",
        parse_mode="HTML",
...
async def paint_receive_link(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    pack_name = m.group(1) if m else (text if re.fullmatch(r"[A-Za-z][A-Za-z0-9_]{4,63}", text) else None)

    if not pack_name:
        await update.message.reply_text(
            "❌ Не распознал. Отправь стикер или ссылку вида <code>t.me/addstickers/ИМЯ</code>",
            parse_mode="HTML",
        )
        return PAINT_SCOPE
...
async def _paint_execute(update: Update, context: ContextTypes.DEFAULT_TYPE, hex_color: str):
    """Выполняет покраску — одного стикера или целого пака."""
    ...
    try:
        if mode == "single":
            ...
            await bot.send_sticker(
                msg.chat_id,
                InputFile(io.BytesIO(result), filename=fname),
            )
            ...
            total = len(stickers)
            is_animated = any(s.is_animated for s in stickers if not s.is_video)
            ...
            for i, s in enumerate(stickers):
                ...
                try:
                    ...
                    if s.is_animated:
                        ...
                        fname = f"sticker_{i}.tgs"
                    else:
                        data_out = recolor_webp(raw, hex_color)
                        fname = f"sticker_{i}.webp"

                    sticker_file = InputFile(io.BytesIO(data_out), filename=fname)
                    emoji = s.emoji or "⭐"
                    input_sticker = InputSticker(sticker=sticker_file, emoji_list=[emoji], format=sticker_format)
                    ...
                    logger.warning("paint pack sticker %d failed: %s", i, e)
            ...
            if pack_created:
                pack_link = f"https://t.me/addstickers/{new_pack_name}"
                ...
                await msg.reply_text(
                    "❌ Не удалось перекрасить ни одного стикера.",
                    parse_mode="HTML",
                    reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🏠 В меню", callback_data="begin")]]),
                )
            context.user_data.pop("paint_sticker", None)

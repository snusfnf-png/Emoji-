...
(
    ...
    ADMIN_DAILY_DATE,     # 26
    # ── Смешивание эмодзи ──
    EMOJI_MIX_FIRST,      # 27 — ожидание первого эмодзи
    EMOJI_MIX_SECOND,     # 28 — ожидание второго эмодзи
) = range(29)
...
async def send_main_menu(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    kb = [
        ...
        [InlineKeyboardButton("🍳  Смешать эмодзи",           callback_data="emoji_mix")],
    ]
    text = (
        ...
        "• <b>Смешать эмодзи</b> — Google Emoji Kitchen"
    )
...
async def cancel_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    active_keys = (
        ...
        "_emoji_mix_active",                         # смешивание эмодзи
    )
    ...
    for k in (
        ...
        "_emoji_mix_active", "_emoji_mix_first",
    ):
...
def _emoji_to_codepoint(emoji_str: str) -> str | None:
    """Конвертирует эмодзи в hex codepoint для Emoji Kitchen API.
    Например: 😊 → 1f60a
    """
    try:
        # Убираем вариационные селекторы (U+FE0F) и ZWJ для получения базового кода
        cps = [f"{ord(c):x}" for c in emoji_str if ord(c) not in (0xFE0F, 0x200D, 0x20E3)]
        if not cps:
            return None
        return "-".join(cps) if len(cps) > 1 else cps[0]
    except Exception:
        return None


async def emoji_mix_start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Запускает процесс смешивания эмодзи."""
    query = update.callback_query
    await query.answer()
    context.user_data["_emoji_mix_active"] = True
    await query.message.reply_text(
        "🍳 <b>Смешать эмодзи</b>\n\n"
        "Отправь первый эмодзи:\n"
        "<i>Например: 😊 😎 🔥 ❤️ 🐱</i>\n\n"
        "/cancel — отмена",
        parse_mode="HTML",
    )
    return EMOJI_MIX_FIRST


async def emoji_mix_receive_first(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Принимает первый эмодзи."""
    text = update.message.text.strip() if update.message.text else ""
    emojis = _extract_emoji(text)
    if not emojis:
        await update.message.reply_text(
            "❌ Не нашёл эмодзи. Отправь один смайлик, например: 😊\n\n/cancel — отмена"
        )
        return EMOJI_MIX_FIRST
    context.user_data["_emoji_mix_first"] = emojis[0]
    await update.message.reply_text(
        f"✅ Первый: {emojis[0]}\n\n"
        "Теперь отправь второй эмодзи:\n\n"
        "/cancel — отмена",
    )
    return EMOJI_MIX_SECOND


async def emoji_mix_receive_second(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Принимает второй эмодзи, запрашивает Emoji Kitchen и отправляет стикер."""
    import aiohttp
    text = update.message.text.strip() if update.message.text else ""
    emojis = _extract_emoji(text)
    if not emojis:
    ...
    e1 = context.user_data.get("_emoji_mix_first", "")
    e2 = emojis[0]

    status = await update.message.reply_text("🍳 Смешиваю...")

    cp1 = _emoji_to_codepoint(e1)
    cp2 = _emoji_to_codepoint(e2)

    if not cp1 or not cp2:
        await status.edit_text("❌ Не удалось обработать эмодзи. Попробуй другие.")
        return CHOOSE_TYPE

    # Пробуем оба порядка (Kitchen иногда зависит от порядка)
    urls = [
        f"https://emojik.vercel.app/s/{cp1}_{cp2}?size=512",
        f"https://emojik.vercel.app/s/{cp2}_{cp1}?size=512",
    ]
    ...
    try:
        ...
        logger.warning("emoji_mix: request error: %s", e)

    if not img_data or len(img_data) < 500:
        await status.edit_text(
            f"😔 Комбинация {e1} + {e2} не найдена в Google Emoji Kitchen.\n\n"
            "Попробуй другие эмодзи — не все комбинации существуют."
        )
        context.user_data.pop("_emoji_mix_first", None)
        context.user_data.pop("_emoji_mix_active", None)
        return CHOOSE_TYPE

    # Конвертируем PNG в стикер 512x512
    try:
        sticker_data = process_image_for_sticker(img_data, "sticker")
    except Exception:
        sticker_data = img_data

    try:
        await status.delete()
        buf = io.BytesIO(sticker_data)
        buf.name = "sticker.png"
        buf.seek(0)
        await update.message.reply_sticker(sticker=buf)
    except Exception as e:
        logger.warning("emoji_mix: send sticker error: %s", e)
        # Fallback — отправить как фото
        try:
            buf = io.BytesIO(img_data)
            buf.seek(0)
            await status.edit_text("⬆️ Результат:")
            await update.message.reply_photo(photo=buf, caption=f"{e1} + {e2} = 🍳")
        except Exception as e2:
            await status.edit_text(f"❌ Ошибка при отправке: {e2}")

    context.user_data.pop("_emoji_mix_first", None)
    context.user_data.pop("_emoji_mix_active", None)

    kb = [
        [InlineKeyboardButton("🍳 Смешать ещё", callback_data="emoji_mix")],
        [InlineKeyboardButton("🏠 В меню",       callback_data="begin")],
    ]
    await update.message.reply_text(
        "Готово! Хочешь смешать ещё?",
        reply_markup=InlineKeyboardMarkup(kb),
    )
    return CHOOSE_TYPE
    ...
    conv = ConversationHandler(
        ...
        states={
            ...
            CHOOSE_TYPE: [
                ...
                CallbackQueryHandler(copy_pack_restart,    pattern="^copy_pack$"),
                CallbackQueryHandler(emoji_mix_start,      pattern="^emoji_mix$"),
            ],
            PACK_NAME: [
            ...
            EMOJI_MIX_FIRST: [
                CommandHandler("cancel", cancel_command),
                CallbackQueryHandler(begin,                 pattern="^begin$"),
                MessageHandler(filters.TEXT & ~filters.COMMAND, emoji_mix_receive_first),
            ],
            EMOJI_MIX_SECOND: [
                CommandHandler("cancel", cancel_command),
                CallbackQueryHandler(begin,                 pattern="^begin$"),
                MessageHandler(filters.TEXT & ~filters.COMMAND, emoji_mix_receive_second),
            ],
            # ── Админ-панель ────────────────────────────────────────────────
            ADMIN_MENU: [

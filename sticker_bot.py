...
(
    ...
    ADMIN_MENU,           # 18
    ADMIN_BROADCAST,      # 19
    ADMIN_SCHEDULED_TIME, # 20
    ADMIN_SCHEDULED_MSG,  # 21
    ADMIN_SET_PRICE,      # 22
    ADMIN_DAILY_DATE,     # 23
) = range(24)
...
async def send_main_menu(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    kb = [
        [InlineKeyboardButton("<tg-emoji emoji-id="5258318251355545562">🖼</tg-emoji>  Создать стикер-пак",      callback_data="create_sticker")],
        [InlineKeyboardButton("<tg-emoji emoji-id="5258262708838472996">✨</tg-emoji>  Создать эмодзи-пак",       callback_data="create_emoji")],
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    text = (
        f"<tg-emoji emoji-id="6035084557378654059">👤</tg-emoji> <b>Профиль {name}</b>\n\n"
        f"<blockquote>"
        f"<tg-emoji emoji-id="5904462880941545555">🛍</tg-emoji>️ Покупок: <b>{purchases}</b>\n"
        f"<tg-emoji emoji-id="6028338546736107668">⭐</tg-emoji> Потрачено: <b>{stars_spent}</b>\n"
        f"<tg-emoji emoji-id="5890937706803894250">📅</tg-emoji> Регистрация: <b>{reg_str}</b>"
        f"</blockquote>"
    )

    kb = [
        [InlineKeyboardButton("Меню <tg-emoji emoji-id="5938537205847822613">🏠</tg-emoji>", callback_data="start_begin"),
         InlineKeyboardButton("Мои паки <tg-emoji emoji-id="5805550320985578625">📁</tg-emoji>", callback_data="start_packs")],
        [InlineKeyboardButton("Покрасить стикер <tg-emoji emoji-id="6050679691004612757">🎨</tg-emoji>", url="https://t.me/Meganaytbot?start=start"),
         InlineKeyboardButton("Поддержка <tg-emoji emoji-id="5938252440926163756">ℹ️</tg-emoji>", url="https://t.me/tntks")],
    ]
...
async def start_packs_cb(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if not packs:
        await query.message.reply_text(
            ...
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("<tg-emoji emoji-id="5938537205847822613">🏠</tg-emoji> В меню", callback_data="start_begin")]]),
        )
        return MAIN_MENU
    ...
    for i, p in enumerate(packs):
        if p.get("plate_pack"):
            icon = "<tg-emoji emoji-id="6050679691004612757">🎨</tg-emoji>" if p["type"] == "sticker" else "📷"
        elif p["type"] == "sticker":
            icon = "<tg-emoji emoji-id="5258318251355545562">🖼</tg-emoji>"
        elif p.get("adaptive"):
            icon = "<tg-emoji emoji-id="6050679691004612757">🎨</tg-emoji>"
        else:
            icon = "<tg-emoji emoji-id="5258262708838472996">✨</tg-emoji>"
        n = p.get("count", 0)
    ...
    kb.append(back_btn("<tg-emoji emoji-id="5938537205847822613">🏠</tg-emoji> В меню", "begin"))
    await query.message.reply_text(
...
async def packs_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if not packs:
        await update.message.reply_text(
            "У тебя пока нет созданных паков.",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("<tg-emoji emoji-id="5938537205847822613">🏠</tg-emoji> В меню", callback_data="begin")]]),
        )
        return MAIN_MENU
...
async def cancel_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    # /cancel работает только когда идёт активный процесс (создание пака, номерного знака, редактирование)
    active_keys = ("creating_type", "plate_country", "selected_pack_index",
                   "new_pack_title", "adm_price_key", "adm_scheduled_msg")
    is_active = any(k in context.user_data for k in active_keys)
    if not is_active:
        # Вне процесса создания — молча игнорируем
        return CHOOSE_TYPE
    # Очищаем данные текущего процесса
...
async def adm_users(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if not users:
        ...
        for uid, reg_at, purchases, stars, _ in users[:50]:
            ...
            lines.append(f"• <code>{uid}</code> | {reg} | <tg-emoji emoji-id="5904462880941545555">🛍</tg-emoji>{purchases} <tg-emoji emoji-id="6028338546736107668">⭐</tg-emoji>{stars}")
        if len(users) > 50:
...
async def adm_stats(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    daily_rows  = DB.get_daily_stats_full(limit=14)
    ...
    lines = [
        ...
        "━━━━━━━━━━━━━━━━",
        "👥 <b>Общее</b>",
        f"  <tg-emoji emoji-id="6035084557378654059">👤</tg-emoji> Пользователей:     <b>{total_users}</b>",
        f"  <tg-emoji emoji-id="5258318251355545562">🖼</tg-emoji> Стикер-паков:      <b>{sp}</b>",
        f"  <tg-emoji emoji-id="5258262708838472996">✨</tg-emoji> Эмодзи-паков:      <b>{ep}</b>",
    ...
    # ── По дням ──
    if daily_rows:
        lines += ["", "━━━━━━━━━━━━━━━━", "<tg-emoji emoji-id="5890937706803894250">📅</tg-emoji> <b>Активность по дням</b> (последние 14)",""]
        # Максимум для мини-бара
        max_active = max((r[1] for r in daily_rows), default=1) or 1
        for day, active, new in daily_rows:
            bar_len  = round((active / max_active) * 8)
            bar      = "█" * bar_len + "░" * (8 - bar_len)
            day_str  = str(day)  # YYYY-MM-DD → оставим как есть
            # Форматируем в ДД.ММ
            try:
                ...
                day_fmt = str(day)[-5:]
            lines.append(
                f"<code>{day_fmt}</code> <code>{bar}</code> 👁{active}"
                + (f"  <i>+{new} new</i>" if new else "")
            )
    else:
        lines += ["", "<tg-emoji emoji-id="5890937706803894250">📅</tg-emoji> <i>Данных по дням пока нет</i>"]
...
# ── Новые по дням (оставляем для совместимости, редиректим на adm_stats) ──────
...
async def adm_schedule_receive_time(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await update.message.reply_text(
        f"✅ Рассылка запланирована на <b>{send_at.strftime('%d.%m.%Y %H:%M')}</b>",
        parse_mode="HTML",
        reply_markup=_admin_main_kb(),
    )
    return ADMIN_MENU
...
# ── Цены ──────────────────────────────────────────────────────────────────────
PRICE_LABELS = {
    "price_plate_sticker": "🚗 Номер → стикер-пак (<tg-emoji emoji-id="6028338546736107668">⭐</tg-emoji>)",
    "price_plate_emoji":   "🚗 Номер → эмодзи-пак (<tg-emoji emoji-id="6028338546736107668">⭐</tg-emoji>)",
}
...
async def adm_prices(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    for key, label in PRICE_LABELS.items():
        ...
        lines.append(f"• {label}: <b>{val} <tg-emoji emoji-id="6028338546736107668">⭐</tg-emoji></b>")
...
async def adm_price_select(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await q.edit_message_text(
        f"✏️ <b>Изменить цену</b>\n\n{label}\nТекущая цена: <b>{cur} <tg-emoji emoji-id="6028338546736107668">⭐</tg-emoji></b>\n\nВведи новое значение (целое число):",
        parse_mode="HTML",
        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("❌ Отмена", callback_data="adm_back")]]),
    )
...
async def adm_price_receive(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await update.message.reply_text(
        f"✅ Цена «{label}» изменена на <b>{text} <tg-emoji emoji-id="6028338546736107668">⭐</tg-emoji></b>",
        parse_mode="HTML",
        reply_markup=_admin_main_kb(),
    )
    return ADMIN_MENU
...
async def adm_maintenance(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if current == "1":
        ...
        status = "🔧 Тех. перерыв <b>включён</b>. Пользователи видят сообщение о работах."
    await q.edit_message_text(
...
async def select_pack(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if pack.get("plate_pack"):
        icon = "<tg-emoji emoji-id="6050679691004612757">🎨</tg-emoji>" if pack["type"] == "sticker" else "📷"
    ...
    paint_label  = "<tg-emoji emoji-id="6050679691004612757">🎨</tg-emoji> Покрасить эмодзи" if is_emoji else "<tg-emoji emoji-id="6050679691004612757">🎨</tg-emoji> Покрасить стикер"
    kb = [
        ...
        *([[InlineKeyboardButton("<tg-emoji emoji-id="5258318251355545562">🖼</tg-emoji> Сменить иконку пака", callback_data="mgmt_icon")]] if not is_plate else []),
...
async def start_create(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if pack_type == "emoji":
        ...
        await query.edit_message_text(
            "<tg-emoji emoji-id="5258262708838472996">✨</tg-emoji> <b>Создание эмодзи-пака</b>\n\n"
...
async def set_adaptive(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await query.edit_message_text(
        "<tg-emoji emoji-id="5258262708838472996">✨</tg-emoji> <b>Создание адаптивного эмодзи-пака</b>\n\n"
...
async def receive_pack_link(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if suffix != raw.lower():
        await update.message.reply_text(
            f"<tg-emoji emoji-id="5938252440926163756">ℹ️</tg-emoji> Ссылка скорректирована: <code>{suffix}</code>", parse_mode="HTML"
        )
    return await _prompt_first_sticker(update, suffix, context)
...
async def save_pack(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    url  = pack_url(pack_name)
    icon = "<tg-emoji emoji-id="5258318251355545562">🖼</tg-emoji>" if pack_type == "sticker" else "<tg-emoji emoji-id="5258262708838472996">✨</tg-emoji>"
    kb   = [
        [InlineKeyboardButton("🔗 Открыть пак", url=url)],
        [InlineKeyboardButton("<tg-emoji emoji-id="5938537205847822613">🏠</tg-emoji> В меню",      callback_data="begin")],
    ]
    await query.edit_message_text(
...
async def mgmt_icon(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    item_label = "эмодзи" if is_emoji else "стикер"
    await query.edit_message_text(
        f"<tg-emoji emoji-id="5258318251355545562">🖼</tg-emoji> <b>Сменить иконку пака</b>\n\n"
...
async def plate_receive_input(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        kb = [
            [InlineKeyboardButton("<tg-emoji emoji-id="6050679691004612757">🎨</tg-emoji> В стикер-пак <tg-emoji emoji-id="6028338546736107668">⭐</tg-emoji>️",  callback_data="plate_buy_sticker"),
             InlineKeyboardButton("📷 В эмодзи-пак <tg-emoji emoji-id="6028338546736107668">⭐</tg-emoji>️",   callback_data="plate_buy_emoji")],
            [InlineKeyboardButton("🔄 Другой номер",        callback_data=f"plate_other_{country}")],
            [InlineKeyboardButton("<tg-emoji emoji-id="5938537205847822613">🏠</tg-emoji> В меню",              callback_data="plate_menu")],
        ]
        try:
...
async def plate_buy_sticker(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await context.bot.send_message(
        chat_id=query.message.chat_id,
        text="Получить этот номер в формате стикера",
    )
...
async def plate_buy_emoji(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await context.bot.send_message(
        chat_id=query.message.chat_id,
        text="Получить этот номер в формате эмодзи",
    )
...
async def plate_successful_payment(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if pack:
        ...
        if err:
            ...
            url  = pack_url(pack["name"])
            icon = "<tg-emoji emoji-id="6050679691004612757">🎨</tg-emoji>" if pack_type == "sticker" else "📷"
            await update.message.reply_text(
    ...
    async def post_init(application):
        ...
        try:
            ...
            await application.bot.set_my_commands([
                ("start",  "<tg-emoji emoji-id="6035084557378654059">👤</tg-emoji> Я"),
                ("menu",   "<tg-emoji emoji-id="5938537205847822613">🏠</tg-emoji> Главное меню"),
                ("packs",  "📋 Паки"),

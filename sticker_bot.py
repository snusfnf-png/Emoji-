...
def _find_or_download_emoji_font():
    import urllib.request
    ...
    font_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "fonts")
    os.makedirs(font_dir, exist_ok=True)
    dl_path = os.path.join(font_dir, "NotoColorEmoji.ttf")
    if os.path.exists(dl_path):
        return dl_path
    try:
        logger.info("Скачиваю шрифт: %s", dl_path)
        ...
        logger.warning("Не удалось скачать NotoColorEmoji: %s", e)
        return None


EMOJI_FONT_PATH = _find_or_download_emoji_font()
# ── Данные регионов ───────────────────────────────────────────────────────────
...
def generate_plate_image(country: str, chars: str, region: str, region_name: str) -> bytes:
    ...
    if country == "RU":
        ...
        if EMOJI_FONT_PATH:
            try:
                fnt_emoji = ImageFont.truetype(EMOJI_FONT_PATH, 109)

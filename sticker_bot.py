...
# ── Премиум эмодзи ────────────────────────────────────────────────────────────
# Для текста сообщений (parse_mode="HTML")
PE_FRAME  = '<tg-emoji emoji-id="525831825135">🖼️</tg-emoji>'
PE_SPARK  = '<tg-emoji emoji-id="525826270883">✨</tg-emoji>'
# Для кнопок — placeholder символы (1 символ = 1 entity offset)
PE_FRAME_BTN = "🖼️"
PE_SPARK_BTN = "✨"


def _premium_btn(text: str, callback_data: str = None, url: str = None) -> dict:
    """Возвращает raw dict кнопки с premium emoji entities для кнопок клавиатуры.
    Заменяет PE_FRAME_BTN и PE_SPARK_BTN на tg-emoji entities.
    """
    entities = []
    result_text = text
    for emoji_str, emoji_id in (
        (PE_FRAME_BTN, "525831825135"),
        (PE_SPARK_BTN, "525826270883"),
    ):
        offset = 0
        while True:
            idx = result_text.find(emoji_str, offset)
            if idx == -1:
                break
            # Считаем UTF-16 offset (Telegram использует UTF-16 единицы)
            utf16_offset = len(result_text[:idx].encode("utf-16-le")) // 2
            utf16_length = len(emoji_str.encode("utf-16-le")) // 2
            entities.append({
                "type": "custom_emoji",
                "offset": utf16_offset,
                "length": utf16_length,
            ...
            offset = idx + len(emoji_str)
    btn = {"text": result_text}
    if entities:
        btn["text_entities"] = entities  # Telegram Bot API поле для entities в кнопке (нестандартное, используем через raw)
    if callback_data:
...
async def _send_or_edit_with_premium_btns(
    bot,
    chat_id: int,
    text: str,
    reply_markup_rows: list,   # list[list[dict]]  — каждый dict из _premium_btn()
    parse_mode: str = "HTML",
    message_id: int = None,    # если задан — edit, иначе send
    reply_to_message_id: int = None,
) -> None:
    """Отправляет/редактирует сообщение с premium emoji в кнопках через raw Bot API."""
    import aiohttp, json as _json
    token = bot.token
    inline_keyboard = reply_markup_rows
    payload = {
        "chat_id": chat_id,
        "text": text,
    ...
    async with aiohttp.ClientSession() as session:
        async with session.post(url, data=payload) as resp:
            ...
            if not data.get("ok"):
                logger.warning("_send_or_edit_with_premium_btns error: %s", data)
            return data
...
async def send_main_menu(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    kb_rows = [
        [_premium_btn(f"{PE_FRAME_BTN}  Создать стикер-пак",  callback_data="create_sticker")],
        [_premium_btn(f"{PE_SPARK_BTN}  Создать эмодзи-пак",  callback_data="create_emoji")],
        [_premium_btn("🚗  Создать номерной знак",             callback_data="create_plate")],
    ]
    text = (
        "👋 <b>Что хочешь создать?</b>\n\n"
        f"• {PE_FRAME} <b>Стикер-пак</b> — обычные стикеры\n"
        f"• {PE_SPARK} <b>Эмодзи-пак</b> — premium emoji\n"
        "• 🚗 <b>Номерной знак</b> — номера стран СНГ"
    )
    if update.callback_query:
        await update.callback_query.answer()
        msg = update.callback_query.message
        await _send_or_edit_with_premium_btns(
            context.bot, msg.chat_id, text, kb_rows,
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    kb = [
        ...
        [InlineKeyboardButton(f"Покрасить стикер {PE_FRAME_BTN}", url="https://t.me/Meganaytbot?start=start"),
...
async def start_begin_cb(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await _send_or_edit_with_premium_btns(
        context.bot, query.message.chat_id, text, kb_rows,
    )
    return CHOOSE_TYPE
...
async def start_packs_cb(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    for i, p in enumerate(packs):
        if p.get("plate_pack"):
            icon = PE_FRAME_BTN if p["type"] == "sticker" else PE_SPARK_BTN
        elif p["type"] == "sticker":
            icon = PE_FRAME_BTN
        elif p.get("adaptive"):
            icon = PE_FRAME_BTN
...
def _stats_text_and_kb(stats: dict, day_users: int, day=None, page: int = 0, total_pages: int = 1) -> tuple:
    ...
    text = (
        ...
        f"{PE_FRAME} Стикер-паков:      <b>{sp}</b>\n"
        f"{PE_SPARK} Эмодзи-паков:      <b>{ep}</b>\n"
...
async def select_pack(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if pack.get("plate_pack"):
        icon = PE_FRAME_BTN if pack["type"] == "sticker" else PE_SPARK_BTN
    ...
    paint_label  = f"{PE_FRAME_BTN} Покрасить эмодзи" if is_emoji else f"{PE_FRAME_BTN} Покрасить стикер"
    kb = [
        ...
        *([[InlineKeyboardButton(f"{PE_FRAME_BTN} Сменить иконку пака", callback_data="mgmt_icon")]] if not is_plate else []),
...
async def start_create(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if pack_type == "emoji":
        ...
        await query.edit_message_text(
            f"{PE_SPARK} <b>Создание эмодзи-пака</b>\n\n"
...
async def set_adaptive(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await query.edit_message_text(
        f"{PE_SPARK} <b>Создание адаптивного эмодзи-пака</b>\n\n"
...
async def save_pack(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    url  = pack_url(pack_name)
    icon = PE_FRAME_BTN if pack_type == "sticker" else PE_SPARK_BTN
    kb   = [
        [InlineKeyboardButton("🔗 Открыть пак", url=url)],
...
async def mgmt_icon(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    item_label = "эмодзи" if is_emoji else "стикер"
    await query.edit_message_text(
        f"{PE_FRAME} <b>Сменить иконку пака</b>\n\n"
...
async def plate_receive_input(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        kb = [
            [InlineKeyboardButton(f"{PE_FRAME_BTN} В стикер-пак ⭐️",  callback_data="plate_buy_sticker"),
             InlineKeyboardButton(f"{PE_SPARK_BTN} В эмодзи-пак ⭐️",  callback_data="plate_buy_emoji")],

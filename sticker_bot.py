...
async def paint_receive_custom_emoji(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    Извлекаем имя пака из первого кастомного эмодзи и запускаем покраску пака.
    """
    msg = update.message
    ...
    status = await msg.reply_text("🔍 Ищу пак для этого эмодзи...")
    try:
        # Получаем информацию о кастомном эмодзи через API
        ...
        if not pack_name:
            await status.edit_text(
                "❌ Этот эмодзи не входит в пак или пак недоступен.\n"
                "Попробуй отправить ссылку: <code>t.me/addemoji/ИМЯ</code>",
                parse_mode="HTML",
            )
            return PAINT_SCOPE
    except Exception as e:
    ...
    last = context.user_data.get("paint_last_color")
    await msg.reply_text(
        f"📦 <b>Эмодзи-пак: <code>{pack_name}</code></b>\n\nВыбери цвет для покраски всего пака:",
        parse_mode="HTML",
        reply_markup=_paint_kb_colors(last),
    )
    return PAINT_COLOR
